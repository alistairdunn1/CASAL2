...
MortalityInstantaneous::MortalityInstantaneous(shared_ptr<Model> model) : Process(model), partition_(model) {
  ...
  method_table_->set_required_columns({PARAM_METHOD, PARAM_CATEGORY, PARAM_SELECTIVITY, PARAM_TIME_STEP, PARAM_U_MAX, PARAM_PENALTY}, false);
...
void MortalityInstantaneous::DoBuild() {
  ...
  for (auto& category : categories_) {
    ...
    category.exploitation_.assign(model_->get_number_of_length_bins(), 0.0);
    category.exp_values_half_m_.assign(model_->get_number_of_length_bins(), 0.0);
    category.selectivity_values_.assign(model_->get_number_of_length_bins(), 0.0);
  }

  for (auto& fishery_category : fishery_categories_) {
    fishery_category.selectivity_values_.assign(model_->get_number_of_length_bins(), 0.0);
  }
  ...
  for (unsigned year_ndx = 0; year_ndx < process_years_.size(); ++year_ndx) {
    ...
    for (unsigned fishery_ndx = 0; fishery_ndx < fisheries_.size(); ++fishery_ndx) {
      ...
      for (unsigned category_ndx = 0; category_ndx < category_labels_.size(); ++category_ndx)
        removals_by_year_fishery_category_[year_ndx][fishery_ndx][category_ndx].resize(model_->get_number_of_length_bins(), 0.0);
    }
  }
}
...
void MortalityInstantaneous::DoExecute() {
  ...
  if ((model_->state() != State::kInitialise
    ...
    for (auto& fishery_category : fishery_categories_) {
      ...
      LOG_FINEST() << "Category is fished in year " << year << " time_step " << time_step_index << ", numbers at length = " << category->data_.size();
     ...
     */
    bool recalculate_length_exploitation = false;
    ...
    for (auto& fishery_iter : fisheries_) {
      ...
      for (auto& fishery_category : fishery_categories_) {
        ...
        for (Double length_exploitation : fishery_category.category_.exploitation_) {
          uobs = uobs > length_exploitation ? uobs : length_exploitation;
        }
      }
    }
    LOG_FINEST() << "Size of fishery_categories_ " << fishery_categories_.size();
    ...
    /**
     * recalculate length exploitation if we triggered penalty
     */
    if (recalculate_length_exploitation) {
      for (auto& category : categories_) {
        LOG_FINE() << "recalculating length exploitation for category " << category.category_label_ << " in time step " << time_step_index;
        for (unsigned i = 0; i < model_->get_number_of_length_bins(); ++i) category.exploitation_[i] = 0.0;
      }
    ...
    /**
     * Calculate the expectation for a proportions_at_length observation
     */
  ...
  for (auto& category : categories_) {
    ...
    for (unsigned i = 0; i < category.category_->data_.size(); ++i) {
      LOG_FINEST() << "category " << category.category_label_ << ": numbers at length = " << category.category_->data_[i] << " length ndx " << i
