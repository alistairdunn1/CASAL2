...
void MarkovianMovement::DoValidate() {
  ...
  parameters_.ValidateVector(PARAM_PROPORTIONS)->GreaterThanOrEqualTo(0.0)->LessThanOrEqualTo(1.0)->ExpandToSameNumberOfElementsAs(PARAM_TO)->SameNumberOfElementsAs(PARAM_TO);
  ...
  for (unsigned i = 0; i < to_category_names_.size(); ++i) {
    ...
    LOG_FINE() << "i = " << i << " estimable parameter = " << category_label << " from  category = " << from_category_names_[i] << " to = " << to_category_names_[i]
               << " selectivity = " << selectivity_names_[i] << " prop = " << proportions_[i];
  ...
  for (auto from_props : from_to_proportions_) {
    ...
    for (auto to_props : from_props.second) sum_props += to_props.second;
    if (fabs(sum_props - 1.0) > 0.001)
      LOG_ERROR_P(PARAM_PROPORTIONS) << "The " << PARAM_FROM << " category labeled " << from_props.first << " has proportions across all " << PARAM_TO
                                     << " categories which sum = " << sum_props << ". This needs to sum to one or at least close to one by 0.0001";
  }
}
...
void MarkovianMovement::DoExecute() {
  ...
  for (unsigned i = 0; from_iter != from_partition_.end() && to_iter != to_partition_.end(); ++from_iter, ++to_iter, ++i) {
    LOG_FINEST() << "category = " << (*from_iter)->name_ << " to category = " << (*to_iter)->name_ << " i = " << i
                 << " prop = " << proportions_by_category_[category_lookup_table_[i]];
    ...
    for (unsigned offset = 0; offset < (*from_iter)->data_.size(); ++offset)
      abundance_to_move_categories_[i][offset]
          = proportions_by_category_[category_lookup_table_[i]] * selectivities_[i]->GetAgeResult(min_age_ + offset, (*from_iter)->age_length_) * (*from_iter)->data_[offset];
  }
  ...
  for (unsigned i = 0; from_iter != from_partition_.end() && to_iter != to_partition_.end(); ++from_iter, ++to_iter, ++i) {
    ...
    for (unsigned offset = 0; offset < (*from_iter)->data_.size(); ++offset) {
      ...
      LOG_FINEST() << "age-ndx - " << offset << " Moving " << abundance_to_move_categories_[i][offset] << " number of individuals, from number " << (*from_iter)->data_[offset]
