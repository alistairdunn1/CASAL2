...
void TagRecaptureByFishery::DoBuild() {
  ...
  for (auto category : split_tagged_category_labels) LOG_FINEST() << category;
...
void TagRecaptureByFishery::Execute() {
  ...
  if (time_step_to_execute_ == current_time_step) {
      ...
      for (; category_iter != partition_iter->end(); ++category_iter) {
        ...
        for (string fishery : method_) {
          ...
          for (unsigned k = 0; k < Removals_at_age.size(); ++k) {
            ...
            LOG_FINE() << "Numbers At Age After Ageing error: " << (*category_iter)->min_age_ + k << " for category " << (*category_iter)->name_ << " " << Removals_at_age[k];
    ...
    LOG_FINEST() << "-----";
    SaveComparison(label_, expected_recaptures_[year][0], observed_recaptures_[year][0], 0.0, AS_DOUBLE(expected_recaptures_[year][0]), 0.0, delta_, 0.0);
