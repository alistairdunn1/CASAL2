...
void AgeLength::DoValidate() {
  ...
  if (sample_type_ == PARAM_AGE) {
    ...
    // first calculate the denominator for unique fish sizes, by sex
  ...
  for (unsigned i = 0; i < category_labels_.size(); ++i) {
    ...
    for (string category_label : split_category_labels) split_category_labels_.push_back(category_label);
  }
  ...
  if (selectivity_labels_.size() != split_category_labels_.size()) {
    LOG_FATAL_P(PARAM_SELECTIVITIES) << "you need to supply a selectivity for each combined category '" << split_category_labels_.size() << "', but only supplied '"
  ...
  for (unsigned i = 0; i < numerator_categories_.size(); ++i) {
    ...
    for (string category_label : split_category_labels) split_numerator_categories_.push_back(category_label);
  }
  ...
  for (string cat : split_numerator_categories_) {
    ...
    if (!this_category_iter.first)
      LOG_FATAL_P(PARAM_NUMERATOR_CATEGORIES) << "the category " << cat << " could not be found in the categories supplied in " << PARAM_CATEGORIES
                                              << ". All categories supplied in " << PARAM_NUMERATOR_CATEGORIES << " must also be in " << PARAM_CATEGORIES << ".";
    if (selectivity_label_for_numerator_ != "") {
      if (selectivity_label_for_numerator_ != selectivity_labels_[this_category_iter.second]) {
        LOG_FATAL_P(PARAM_SELECTIVITIES)
            << "The selectivities supplied for numerator categories are not the same. All numerator categories must have the same selectivity. Category " << cat
            << " has selectivity " << selectivity_labels_[this_category_iter.second] << ", but an earlier numerator category had the following selectivity label "
            << selectivity_label_for_numerator_;
      }
    }
        ...
        if (current_age_length_label == "") {
          ...
          if ((*category_iter)->age_length_->label() != current_age_length_label) {
            if (!parameters_.Get(PARAM_NUMERATOR_CATEGORIES)->has_been_defined()) {
              LOG_ERROR_P(PARAM_CATEGORIES) << "The categories supplied have different age-length objects. We recommend specifying " << PARAM_NUMERATOR_CATEGORIES
                                            << " with categories that have a common age-length class";
            } else {
...
void AgeLength::Execute() {
  ...
  for (unsigned category_offset = 0; category_offset < category_labels_.size(); ++category_offset, ++partition_iter) {
    ...
    auto   category_iter = partition_iter->begin();
    // this will loop over individual combined categories and store the necessary information.
    for (unsigned cached_counter = 0; category_iter != partition_iter->end(); ++category_iter, ++cached_counter) {
      fill(numbers_at_age_[cached_counter].begin(), numbers_at_age_[cached_counter].end(), 0.0);
      for (unsigned data_offset = 0; data_offset < (*category_iter)->data_.size(); ++data_offset) {
        select_value = selectivities_[all_category_iter]->GetAgeResult(data_offset + model()->min_age(), age_length_ptr_);
  ...
  if (actual_sample_type_ == SampleType::kRandom || actual_sample_type_ == SampleType::kAge) {
    ...
    for (unsigned i = 0; i < n_fish_; ++i) {
      numerator           = 0.0;
      denominator         = 0.0;
      individual_age_iter = individual_ages_[i] - model()->min_age();
      ...
      if (!apply_ageing_error_) {
        ...
        if (numerator_selectivity_->IsSelectivityLengthBased()) {
          LOG_FINEST() << "N = " << numbers_at_age_numerator_[individual_age_iter]
                       << " pdf = " << age_length_ptr_->get_pdf_with_sized_based_selectivity(individual_ages_[i], individual_lengths_[i], year, time_step, numerator_selectivity_);
          numerator = numbers_at_age_numerator_[individual_age_iter]
                      * age_length_ptr_->get_pdf_with_sized_based_selectivity(individual_ages_[i], individual_lengths_[i], year, time_step, numerator_selectivity_);
        } else {
        ...
        for (unsigned age_iter = 0; age_iter < age_spread_; ++age_iter) {
          if (ageing_error_matrix_[age_iter][individual_age_iter] > 1e-4) {
            if (numerator_selectivity_->IsSelectivityLengthBased()) {
              LOG_FINEST() << "N " << numbers_at_age_numerator_[age_iter] << " mis " << ageing_error_matrix_[age_iter][individual_age_iter] << " pdf "
                           << age_length_ptr_->get_pdf_with_sized_based_selectivity(age_iter + model()->min_age(), individual_lengths_[i], year, time_step, numerator_selectivity_);
              numerator += numbers_at_age_numerator_[age_iter] * ageing_error_matrix_[age_iter][individual_age_iter]
                           * age_length_ptr_->get_pdf_with_sized_based_selectivity(age_iter + model()->min_age(), individual_lengths_[i], year, time_step, numerator_selectivity_);
            } else {
              LOG_FINEST() << "N " << numbers_at_age_numerator_[age_iter] << " mis "
                           << ageing_error_matrix_[age_iter][individual_age_iter];  // << " pdf " << age_length_ptr_->get_pdf_with_sized_based_selectivity(age_iter +
                                                                                    // model_->min_age(), individual_lengths_[i], year, time_step, numerator_selectivity_);
              LOG_FINEST() << "age-length label = " << age_length_ptr_->label();
              LOG_FINEST() << "pdf = " << age_length_ptr_->get_pdf(age_iter + model()->min_age(), individual_lengths_[i], year, time_step);
              numerator += numbers_at_age_numerator_[age_iter] * ageing_error_matrix_[age_iter][individual_age_iter]
                           * age_length_ptr_->get_pdf(age_iter + model()->min_age(), individual_lengths_[i], year, time_step);
            }
          }
        }
      }
      ...
      SaveComparison(numerator_categories_[0], individual_ages_[i], individual_lengths_[i],
                     utilities::math::ZeroFun(numerator, delta_) / utilities::math::ZeroFun(denominator, delta_), 1.0, 0.0, 1.0, 0.0, delta_, 0.0);
    }
        ...
        for (unsigned cached_counter = 0; category_iter != partition_iter->end(); ++category_iter, ++cached_counter) {
          for (unsigned age_iter = 0; age_iter < age_spread_; ++age_iter) {
            if (selectivities_[cached_counter]->IsSelectivityLengthBased()) {
              // LOG_FINEST() << "age = " << age_iter << " N = " << numbers_at_age_[cached_counter][age_iter] << " pdf = " <<
              // age_length_ptr_->age_length_->get_pdf_with_sized_based_selectivity(age_iter + model_->min_age(), unique_lengths_[l], year, time_step,
              // selectivities_[cached_counter]);
              numbers_by_unique_size_[l]
                  += numbers_at_age_[cached_counter][age_iter]
                     * age_length_ptr_->get_pdf_with_sized_based_selectivity(age_iter + model()->min_age(), unique_lengths_[l], year, time_step, selectivities_[cached_counter]);
              LOG_FINEST() << numbers_by_unique_size_[l];
            } else {
              // LOG_FINEST() << "age = " << age_iter << " N = " << numbers_at_age_[cached_counter][age_iter] << " pdf = " << age_length_ptr_->age_length_->get_pdf(age_iter +
              // model_->min_age(), unique_lengths_[l], year, time_step);
              numbers_by_unique_size_[l]
                  += numbers_at_age_[cached_counter][age_iter] * age_length_ptr_->get_pdf(age_iter + model()->min_age(), unique_lengths_[l], year, time_step);
            }
          }
        }
      }
        ...
        for (unsigned age_iter = 0; age_iter < age_spread_; ++age_iter) {
          if (ageing_error_matrix_[age_iter][individual_age_iter] > 1e-4) {
            if (numerator_selectivity_->IsSelectivityLengthBased()) {
              ...
              LOG_FINE() << numbers_at_age_numerator_[age_iter] << " mis " << ageing_error_matrix_[age_iter][individual_age_iter]
                         << " get pdf = " << age_length_ptr_->get_pdf(age_iter + model()->min_age(), individual_lengths_[i], year, time_step);
              numerator += numbers_at_age_numerator_[age_iter] * ageing_error_matrix_[age_iter][individual_age_iter]
                           * age_length_ptr_->get_pdf(age_iter + model()->min_age(), individual_lengths_[i], year, time_step);
            }
          }
