...
void ProportionsAtLength::DoBuild() {
  ...
  LOG_FINE() << "number_bins_ = " << number_bins_ << " model length bins = " << model()->get_number_of_length_bins();
...
void ProportionsAtLength::Execute() {
  ...
  auto     it       = std::find(years_.begin(), years_.end(), model()->current_year());
  unsigned year_ndx = distance(years_.begin(), it);
  LOG_FINE() << "Year = " << model()->current_year() << " year ndx = " << year_ndx;
  /**
...
void ProportionsAtLength::CalculateScore() {
  ...
  if (model()->run_mode() == RunMode::kSimulation) {
    if (model()->get_simulation_iterator() == 0) {
