 ...
 */
TEST_F(InternalEmptyLengthModel, Observation_Length_TagRecapture_By_Length_TwoSexComplex) {
  const std::string observation_definition =
      ...
      tagged_categories immature.male immature.female
      tagged_selectivities logistic_five logistic_six
      time_step step_two
      detection 0.9
  ...
  unsigned index = 0;
  EXPECT_EQ("immature.male", comparisons[index].category_);
  ...
  EXPECT_DOUBLE_EQ(0, comparisons[index].expected_);
  ...
  EXPECT_DOUBLE_EQ(73.277257867700513, comparisons[index].score_);
  ...
  EXPECT_DOUBLE_EQ(9.9999999999999994e-12, comparisons[index].delta_);
  EXPECT_DOUBLE_EQ(123.43335462565582, comparisons[index].score_);
}
