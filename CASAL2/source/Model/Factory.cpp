...
base::Object* Factory::CreateObject(const string& object_type, const string& sub_type, PartitionType partition_type) {
  ...
  if (lwr_object_type == PARAM_ADDITIONAL_PRIOR)
    return additionalpriors::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_AGEING_ERROR || lwr_object_type == PARAM_AGEING_ERRORS)
    return ageingerrors::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_AGE_LENGTH || lwr_object_type == PARAM_AGE_LENGTHS)
    return agelengths::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_AGE_WEIGHT)
    return ageweights::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_ASSERT)
    return asserts::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_CATCHABILITY)
    return catchabilities::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_CATEGORIES)
    return current_model->categories();
  else if (lwr_object_type == PARAM_DERIVED_QUANTITY || lwr_object_type == PARAM_DERIVED_QUANTITIES)
    return derivedquantities::Factory::Create(current_model, lwr_object_type, lwr_sub_type, partition_type);
  else if (lwr_object_type == PARAM_ESTIMATE)
    return estimates::creators::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_GROWTH_INCREMENT || lwr_object_type == PARAM_GROWTH_INCREMENTS)
    return growthincrements::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_PARAMETER_TRANSFORMATION)  // not the same as code class name, easier for users to understand
    return addressabletransformations::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_INITIALISATION_PHASE || lwr_object_type == PARAM_INITIALISATION_PHASES)
    return initialisationphases::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_LENGTH_WEIGHT || lwr_object_type == PARAM_LENGTH_WEIGHTS)
    return lengthweights::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_LIKELIHOOD)
    return likelihoods::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_MINIMIZER)
    return minimisers::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_MCMC)
    return mcmcs::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_MODEL)
    return current_model.get();
  else if (lwr_object_type == PARAM_OBSERVATION)
    return observations::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_PENALTY)
    return penalties::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_PROCESS || lwr_object_type == PARAM_PROCESSES)
    return processes::Factory::Create(current_model, lwr_object_type, lwr_sub_type, partition_type);
  else if (lwr_object_type == PARAM_PROFILE)
    return profiles::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_PROJECT || lwr_object_type == PARAM_PROJECTS)
    return projects::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
    ...
    return reports::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_SELECTIVITY || lwr_object_type == PARAM_SELECTIVITIES)
    return selectivities::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
    ...
    return timesteps::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
  else if (lwr_object_type == PARAM_TIME_VARYING)
    return timevarying::Factory::Create(current_model, lwr_object_type, lwr_sub_type);
