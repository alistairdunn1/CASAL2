...
void CommandLineParser::Parse(int argc, char* argv[], RunParameters& options) {
  ...
  oDesc.add_options()
    ...
    ("query", value<string>(), "Query an object type to view its syntax description. Use [arg] = object_type.sub_type, e.g., process.recruitment_constant")
    ("nostd", "Do not print the standard header report")
    ("loglevel,L", value<string>(), "Set message log level. Defaults to [arg] = info. See the User Manual for further options")
