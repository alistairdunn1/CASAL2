...
public:
    ...
    for (unsigned year_iter = 0; year_iter < age_length_matrix_years_.size(); ++year_iter) {
      ...
      for (unsigned time_step_ndx = 0; time_step_ndx < model->time_steps().size(); ++time_step_ndx) {
        ...
        for (unsigned age_iter = 0; age_iter < model->age_spread(); ++age_iter) {
          age_length_transition_matrix_[year_iter][time_step_ndx][age_iter].resize(model->length_bins().size(), 0.0);
  ...
  const Double Mock_get_age_length_probability(unsigned year, unsigned time_step, unsigned age, unsigned length_bin_ndx) {
...
TEST(AgeLengths, BuildAgeLengthProportions_normal) {
  ...
  */
  vector<Double> expected = {0,         0,         1e-07,     3e-07,     1.3e-06,   4.6e-06,   1.58e-05,  4.95e-05,  0.0001435, 0.0003843, 0.0009515, 0.0021771,
                             0.0046035, 0.0089963, 0.0162479, 0.0271202, 0.0418358, 0.0596439, 0.0785862, 0.0956949, 0.1076949, 0.1120121, 0.1076709, 0.0956522,
                             0.0785335, 0.0595906, 0.0417891, 0.0270839, 0.0162225, 0.0089802, 0.0045942, 0.0021722, 0.0009492, 0.0005977};
...
TEST(AgeLengths, BuildAgeLengthProportions_lognormal) {
  ...
  * Running under: Windows 10 x64 (build 18363)
  ...
  round(expect_probs, 7)
  */
  vector<Double> expected = {0,         0,         0,         0,         0,         0,         1e-07,     9e-07,     7.5e-06,   4.5e-05,  0.0002116, 0.0007986,
                             0.0024716, 0.0063963, 0.0140816, 0.0267734, 0.0445554, 0.0656767, 0.0866653, 0.1033531, 0.1123421, 0.112157, 0.1035552, 0.0889789,
                             0.0715525, 0.0541267, 0.0386966, 0.0262574, 0.016976,  0.0104946, 0.0062238, 0.0035514, 0.0019551, 0.0020955};
