...
void HarvestStrategyRampU::DoValidate() {
  ...
  if (reference_index_years_.size() == 0) {
    ...
    // TODO: update to have a better way of defining we need the initialisation phase value
    reference_index_years_.resize(1, 0);
  } else {
    ...
    vector<unsigned> index_years;
    for (unsigned i = 0; i < reference_index_years_.size(); ++i) {
      if (std::find(index_years.begin(), index_years.end(), reference_index_years_[i]) == index_years.end())
        index_years.push_back(reference_index_years_[i]);
      else
        LOG_ERROR_P(PARAM_REFERENCE_INDEX_YEARS) << ": The year " << reference_index_years_[i] << " occurs more than once. Please check the input file";
    }
  }
...
void HarvestStrategyRampU::DoUpdate() {
  ...
  if (reference_index_years_.size() == 0) {
    // get the reference_index biomass for the initialisation phase value
    ref_biomass = reference_index_->GetValue(index_year) / reference_index_->GetLastValueFromInitialisation(initialisation_phase_);
  } else {
    // else get mean of the reference_index biomass for defined years
    for (unsigned i = 0; i < reference_index_years_.size(); ++i) {
      ref_biomass += biomass_index_->GetValue(reference_index_years_[i]);
    }
    ref_biomass /= reference_index_years_.size();
    // then standardise current value to provide the index
    ref_biomass = reference_index_->GetValue(index_year) / ref_biomass;
  }
