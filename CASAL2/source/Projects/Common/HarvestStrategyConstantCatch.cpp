...
void HarvestStrategyConstantCatch::DoUpdate() {
  ...
  LOG_FINE() << "HarvestStrategyConstantCatch: u=" << catch_ << ", biomass_index=" << biomass << " in year=" << current_model->current_year() << " using index_year=" << index_year
  ...
  if (do_update) {  // its in year_delta, so do an update
    this_catch_  = (((biomass - previous_biomass) / previous_biomass * alpha_) + 1) * catch_ * multiplier_by_year_[current_model->current_year()];
  ...
  LOG_FINE() << "HarvestStrategyConstantCatch: DoUpdateFunc in year=" << current_model->current_year() << ", and with biomass_index=" << biomass << " and catch value=" << value_;
