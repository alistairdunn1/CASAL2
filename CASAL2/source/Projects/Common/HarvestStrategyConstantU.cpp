/**
 * @file HarvestStrategyConstantU.cpp
 * @author  A Dunn
 * @date 01/04/2024
 * @section LICENSE
 *
...
// headers
#include "HarvestStrategyConstantU.h"
...
HarvestStrategyConstantU::HarvestStrategyConstantU(shared_ptr<Model> model) : Project(model), model_(model) {
  ...
  parameters_.Bind<Double>(PARAM_BIOMASS_INDEX_SCALAR, &biomass_index_scalar_, "The biomass index re-scaling parameter")->set_default_value(1.0);
  ...
  parameters_.Bind<unsigned>(PARAM_FIRST_YEAR, &first_year_, "The first year in which to consider an update using the harvest strategy rule")->set_default_value(0);
  // clang-format on
}
 ...
 * Validate
 */
void HarvestStrategyConstantU::DoValidate() {
  parameters_.Validate(PARAM_BIOMASS_INDEX_SCALAR)->GreaterThan(0.0);
  parameters_.Validate(PARAM_U)->GreaterThanOrEqualTo(0.0)->LessThanOrEqualTo(1.0);
 ...
 * Reset
 */
void HarvestStrategyConstantU::DoReset() {
  update_counter_ = 0;
  last_catch_     = current_catch_;
}
