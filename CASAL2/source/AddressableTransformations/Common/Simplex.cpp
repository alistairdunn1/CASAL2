...
Simplex::Simplex(shared_ptr<Model> model) : AddressableTransformation(model) {
  parameters_.Bind<bool>(PARAM_SUM_TO_ONE, &sum_to_one_,
                         "if true, then the parameter vector in natural space will sum to 1, otherwise it will sum to a value equal to length(parameter)", "", true);
...
void Simplex::DoBuild() {
  ...
  // iterate over the simplex estimates and check they are active,
  ...
  if (simplex_input_supplied_) {
    ...
    if (not sum_to_one_) {
      for (unsigned i = 0; i < restored_values_.size(); ++i) restored_values_[i] = unit_vector_[i] * n_param_double_;
    }
  } else {
    ...
    if (sum_to_one_) {
      if (!utilities::math::IsOne(total_))
        LOG_ERROR_P(PARAM_PARAMETERS) << "You have specified a sum_to_one parameter but your parameters sum to " << total_;
      for (unsigned i = 0; i < init_values_.size(); ++i) {
...
void Simplex::DoRestore() {
  ...
  // For details on how the jacobian from Stan manual translates to the code below.
  for (int k = 0; k < Km1_; ++k) {
    adj_y_k = simplex_parameter_[k] - cache_log_k_value_[k];
    zk_[k]  = utilities::math::invlogit(adj_y_k);
    LOG_FINE() << "k = " << k << " zk_[k] " << zk_[k] << " yk = " << simplex_parameter_[k] << " stick length = " << stick_length
               << " cache_log_k_value_[k] = " << cache_log_k_value_[k] << " adj_y_k = " << adj_y_k;
...
void Simplex::RestoreForObjectiveFunction() {
  LOG_FINE();
  if (prior_applies_to_restored_parameters_) {
    for (unsigned i = 0; i < simplex_parameter_.size(); ++i) simplex_parameter_[i] = cached_simplex_values_for_objective_function_restore_[i];
  }
}
...
void Simplex::FillReportCache(ostringstream& cache) {
  ...
  for (unsigned i = 0; i < simplex_parameter_.size(); ++i) cache << simplex_parameter_[i] << " ";
  cache << REPORT_EOL;
