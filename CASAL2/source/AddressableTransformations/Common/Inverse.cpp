 ...
 * Report stuff for this transformation
 */
void Inverse::FillTabularReportCache(ostringstream& cache, bool first_run, const string& sep) {
  LOG_FINEST() << "FillTabularReportCache";
  if (first_run) {
    cache << PARAM_INVERSE_PARAMETER << sep << parameter_labels_[0] << sep << "negative_log_jacobian" << REPORT_EOL;
  }
  cache << inverse_value_ << sep << restored_values_[0] << sep << jacobian_ << REPORT_EOL;
}
} /* namespace addressabletransformations */
} /* namespace niwa */
