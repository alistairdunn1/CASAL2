...
void Nuisance::DoBuild() {
  ...
  has_prior      = model()->managers()->additional_prior()->HasAdditionalPriorExcludingType(parameter, PARAM_RATIO);

  LOG_FINE() << " has prior = " << has_prior;
  if (has_prior) {
    // Obtain a pointer to the estimate
    AdditionalPrior* additional_prior = model()->managers()->additional_prior()->GetAdditionalPriorExcludingType(parameter, PARAM_RATIO);
    if (!additional_prior)
