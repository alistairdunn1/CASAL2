...
DoubleNormalSS3::DoubleNormalSS3(shared_ptr<Model> model) : Selectivity(model) {
  parameters_.Bind<Double>(PARAM_PEAK, &peak_, "beginning size (or age) for the plateau (in cm or year).");
  parameters_.Bind<Double>(PARAM_Y0, &first_sel_val_, "Selectivity at first bin, as logistic between 0 and 1");
  parameters_.Bind<Double>(PARAM_Y1, &last_sel_val_, "Selectivity at last bin, as logistic between 0 and 1");
  ...
  parameters_.Bind<Double>(PARAM_L, &min_x_val_, "The low value (L)");
  parameters_.Bind<Double>(PARAM_H, &max_x_val_, "The high value (H)");
...
void DoubleNormalSS3::DoValidate() {
  ...
  parameters_.Validate(PARAM_H)->GreaterThan(1.0);
