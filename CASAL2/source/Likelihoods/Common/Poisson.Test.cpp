...
TEST(Likelihood, Poisson) {
  ...
  EXPECT_NEAR(1.626387, comparison_list[0][0].score_, 0.001) << " obs = " << comparison_list[0][0].observed_
                                                             << " exp = " << comparison_list[0][0].expected_;  // -1.0 * dpois(3, 2.2, log = T)
  EXPECT_NEAR(5.075884, comparison_list[1][0].score_, 0.001) << " obs = " << comparison_list[1][0].observed_
                                                             << " exp = " << comparison_list[1][0].expected_;  // -1.0 * dpois(5, 1.2, log = T)
  EXPECT_NEAR(0.3, comparison_list[2][0].score_, 0.001) << " obs = " << comparison_list[2][0].observed_
                                                        << " exp = " << comparison_list[2][0].expected_;  // -1.0 * dpois(1, 0.3, log = T)
}
