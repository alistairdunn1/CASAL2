  ...
  for (; iterator != comparisons.end(); ++iterator) {
    ...
    for (observations::Comparison& comparison : iterator->second) {
      ...
      if (this_error_val != check_error_val)
        LOG_WARNING() << "Found N for multinomial in year " << iterator->first << " had N = " << this_error_val << " which differend with the previous N = " << check_error_val
                      << ". When simualting from multinomial need a single N for all bins in the composition. We are just using the first N and ignoreing the others";
      counter++;
    }
