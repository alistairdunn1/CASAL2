...
\item [\texttt{-t [-{}-tabular]}] Print \command{report} in tabular format (see Section \ref{sec:Tabular} for more information).
\item [\texttt{-T [-{}-tabular-tsv]}] Print tabular-format \command{report}s with \emph{tab}-separated data rows instead of space-separated rows (see Section \ref{sec:Tabular} for more information). This flag implies \texttt{-{}-tabular}; it does not need to be combined with \texttt{-t}. Tab-separated output is substantially faster to import into \R\ for large MCMC chains (e.g.~using \texttt{data.table::fread}).
...
Note that if the standard error is redirected to a file (see Section \ref{sec:RedirectingStandardOut}), then the exit status is  redirected to that file. Hence, if calling \CNAME\ from other software (e.g., \R) with a command line that redirects the standard error, the exit status and any messages from \CNAME\ can be found in the file that contains the standard error.
