...
Observations for a process are indicated by their type --- these use the word process as a part of the type name, e,.g., \commandsubarg{observation}{type}{abundance} is an observation of relative abundance that occurs during a mortality block within a time step, and \commandsubarg{observation}{type}{process\_abundance} is a observation  of relative abundance that occurs during a process within a time step.
...
This section describes the algorithms \CNAME\ uses to derive nuisance (analytical) catchability coefficients $q$s (see Section~\ref{syntax:Catchability-Nuisance} for additional details on syntax). From the user's point of view, the essence is that you can use nuisance $q$s, providing that the additional prior on $q$ is one of the following:
\begin{itemize}
	\item Uniform 
	\item Uniform-log
...
The scenarios in which the nuisance catchability $q$ can be used are described in  Table~\ref{tab:nus_overview}.
...
This last term is $0$ for a uniform prior on $q$, $1/q$ for a log-uniform prior, and $\frac{1}{q}\bigg( 1.5 + \frac{log(q) - log(\mu_q)}{\sigma_q^2}$ for a lognormal prior, where $\mu_q$ and $c_q$ are the mean and CV of the prior on $q$, respectively, and $\sigma_q = \sqrt{log(1+c_q^2)}$. Since the prior is uniform, the equation for $\hat q$ is the same as the maximum likelihood estimation.
...
An example of specifying the syntax and an equivalent additional prior is:
