\section{\I{The observation section: observations and their likelihoods} \label{sec:Observation}}

The command and subcommand syntax for the observation section is given in Section \ref{syntax:Observation}.

\subsection{\I{Observations}\index{Observations}}

The objective function calculates the 'goodness-of-fit' to the observed values provided by an observation. Observed values are supplied at an instance in time over a group of categories or aggregated categories. In addition, an observation can contain observed values made over a range of years, at the same time in the model annual cycle and in the same format each year. Examples of such time series data types include abundance indices, commercial \ifAgeBased catch-at-age or \fi catch-at-length frequencies, and survey \ifAgeBased numbers-at-age or \fi numbers-at-length.
...
There are two main types of observations available in \CNAME. The first are time step observations --- these are associated with the mortality block within a time step. The second are process observations --- these are associated with a specific process that occurs within a specific time step during the annual cycle (e.g., a fishing mortality event that occurs in a specific time step, see Section \ref{sec:Process-Mortality} for more information on the definition of mortality blocks within time steps). 

Process observations are indicated by their type and use the keyword \emph{process} with a trailing underscore (i.e., \argument{process\_}) as the initial part of the type name to distinguish them from time step observations. For example,  \commandsubarg{observation}{type}{abundance} is a time step observation of abundance that occurs during a mortality block within a specific time step, and \commandsubarg{observation}{type}{process\_abundance} is a process observation of abundance that occurs during the named process within a time step.

Most observation types that are time step observations can also be process observations, as long as their name is prepended with \argument{process\_} and the process specific subcommands are used. And not all process observations can be time step observations (for example, process removals by \ifAgeBased age\else length\fi). When the observations is a time step observation, the time step and proportion of the time step that has occurred must be specified. When the observation is a process observation, the process and time step that the process occurs in must be specified, along with the proportion of the process that has occurred.

For each observation, the observed values and their associated error values (i.e., the sample sizes or CVs for the observed values) must be specified in either a single table (\subcommand{table obs} for example in the case of abundance observations) or in two tables (\subcommand{table obs} and \subcommand{table error\_values} for example in the case of \ifAgeBased proportions-at-age\else proportions-at-length\fi). 

A robustification constant is used in the likelihood (\argument{delta}) to avoid divide by zero errors. This defaults to 1e-11. \index{robustification constant}\index{Divide by zero in likelihoods!delta}

\subsection{\I{Time step observations} \label{sec:TimeStepObservations} \label{sec:MortalityBlockObservations}}

The likelihood for all time step observations is calculated in a similar manner, by using linear interpolation between the expected value for the observation at the start and end of the mortality block within the specified time step, and then calculating the likelihood of the observations and resulting expected values. First, the expected values are calculated at both the beginning and the end of the mortality block. Then the final expected values are calculated using linear interpolation of these values using the proportion of the time step specified by the subcommand \subcommand{time\_step\_proportion}. The default value is typically 0.5, i.e., defined as 'halfway through the mortality within the time step'.

The time step proportion is always defined and can be used to account for any mortality that may have occurred during the time step before the instance in time when the observation is assumed to occur, for example, if a survey occurs at the start or end of the time step specifying the fishing season when modelling a fish population, then the time step proportion would be 0.0 or 1.0 respectively.

The time step observation types that can be process observations are:

\begin{itemize}
	\item \subcommand{abundance}
	\item \subcommand{biomass}
	\item \subcommand{derived\_quantity}
	\item \subcommand{proportions\_migrating}
	\ifAgeBased
	...
	\ifAgeBased
	\item \subcommand{proportions\_mature\_by\_age}
	\item \subcommand{age\_length}
	\fi
\end{itemize}
	
The time step observation types that cannot be process observations are:
\begin{itemize}
	\ifAgeBased
...
\subsection{\I{Process observations} \label{sec:ProcessObservations}}

As with time step observations, the likelihood for all process observations is calculated in a similar manner, by using linear interpolation between the expected value for the observation at the start and end of the specified process within the specified time step, and then calculating the likelihood of the observations given the resulting expected values. The key difference between time step and process observations are that time step observations use the values from the start and end of a mortality block within a time step, while process observations use the absolute difference between the expected values at the start and end of a process. First, the expected values are calculated at both the beginning and the end of the process. Then the final expected values are calculated using linear interpolation of these values using the proportion specified by the subcommand \subcommand{process\_proportion}. The default value is typically 0.5, i.e., defined as 'halfway through the process'.

The process proportion is always defined and can be used to account for any mortality that may have occurred during the process before the instance in time when the observation is assumed to occur, for example, for fishery mortality this could be the removals at the start mid-point, or end of the process.

\ifAgeBased
The observations that are only process observations are:
% One column version
\begin{itemize}
	\item \subcommand{process\_removals\_by\_age}
	\item \subcommand{process\_removals\_by\_age\_retained}
	\item \subcommand{process\_removals\_by\_age\_retained\_total}
	\item \subcommand{process\_removals\_by\_length}
	\item \subcommand{process\_removals\_by\_length\_retained}
	\item \subcommand{process\_removals\_by\_length\_retained\_total}
	\item \subcommand{process\_proportions\_migrating}
    \item \subcommand{tag\_recapture\_by\_fishery}
\end{itemize}
\fi

\subsection{Descriptions of observations}

\subsubsection{\I{Abundance or biomass observations}} \label{sec:Observation-Abundance} \label{sec:Observation-Biomass}

Abundance (or biomass) observations are observations of either a relative or absolute number (or biomass) of individuals from a set of categories after applying a selectivity. These can be observations of either a time-step or a process. 

The abundance and biomass observation classes are the same, except that a biomass observation will use the biomass as the observed (and expected) value (calculated from mean weight of individuals within each \ifAgeBased age \else length \fi and category) while an abundance observation is the number of individuals.

Each observation is for a given year and time-step, for some selected \ifAgeBased age \else length \fi classes of the population (for a range of \ifAgeBased age \else length \fi classes multiplied by a selectivity), for a category or for aggregated categories. The label of the catchability coefficient $q$ is required;  $q$ can either be estimated of fixed. To use as an estimate of absolute abundance or biomass, define a catchability of $q=1$. Catchabilities can be estimated as either free parameters or as nuisance parameters (see Section \ref{subsec:catchabilities}).
...
\CNAME\ calculates the expected values by summing over the defined \ifAgeBased age \else length \fi classes (via the \ifAgeBased age \else length \fi range and selectivity) and categories at both the beginning and end of either a mortality block for a time-step observation or with9ion a specified process for a process observation. \CNAME\ will approximate the expectation part way through the mortality block or process using the \subcommand{time\_step\_proportion} or \subcommand{process\_proportion} respectively. 
...
Where $E_{i,1}$ is the expectation at the beginning of time step or process, and $E_{i,2}$ is the expectation at the end of the time-step. $S_a$ is the selectivity for \ifAgeBased age \else length \fi $a$ and category $c$. If there is no mortality related to this observation then $E_i$ which is used in the likelihood contribution is $E_{i,1}$. If this was a biomass observation, then $N_{a,c,i,1}$ in Equations~\eqref{eq:expec_1} and~\eqref{eq:expec_2} is replaced with $N_{a,c,i,1} \bar{w}_{a,c}$, where $\bar{w}_{a,c}$ is the mean weight of category $c$ at \ifAgeBased age \else length \fi $a$. If the user wishes to apply 100\% mortality then $E_i = E_{i,2}$.
...
\ifAgeBased
\subsubsection{\I{Proportions-at-age}}\label{sec:Observation-ProportionsAtAge}
...
For example, if the age range was 3 to 10, then eight proportions should be supplied, one proportion for each of the ages 3, 4, 5, 6, 7, 8, 9, and 10. The expected values will be the expected proportions of males within each of these age classes (after omitting males aged less than 3 and older than 10), after applying a selectivity at the year and time-step specified. The supplied vector of proportions (i.e., in this example, the eight proportions) must sum to one, which is evaluated with a default tolerance of 0.001.
...
\subsubsection{\I{Proportions-at-length}} \label{sec:Observation-ProportionsAtLength}
...
\subsubsection{\I{Age-length}}\label{sec:Observation-AgeLength}
...
A difference between CASAL and \CNAME\ is how to deal with sexes and categories. In CASAL sex was a hard coded attribute so it could generate numbers at age for a sex without any input from the user. As \CNAME\ has user supplied categories, the categories that share the same age-length relationship can be defined using the \subcommand{numerator\_categories}. This is an optional parameter, and used when sex is an attribute in the partition along with other attributes (e.g., maturity or tagging). If the subcommand \subcommand{numerator\_categories} is not specified, then the default is the categories supplied in \subcommand{categories}. 

The categories defined by \subcommand{numerator\_categories} are denoted by \(c^*\), where as categories defined by \subcommand{categories} represent the available population in the sampling process denoted by \(c\).
...
\begin{table}[h!]
	...
	\begin{tabular}{c c c c} 
		...
		\cline{3-4}
		\multicolumn{2}{c}{} & With ageing error & Without ageing error\\
		\hline
...
\]
The integrals in these equations are calculated by discrete approximation (using 5 points by default, but this can be modified using \subcommand{n\_quant}), in a similar way as length-based selectivities are converted to age-based selectivities.

The observed values are a one and the expected value is the probability (\(P(a,l)\))

For example, for a sexed model with the categories male and female, then for an age-length observation of type \subcommand{random\_at\_size} for males would be defined as:
{\small{\begin{verbatim}
...
See Section~\ref{syntax:Observation-AgeLength} for the syntax definitions for \commandsubarg{observation}{type}{age\_length} subcommands.
\fi

\subsubsection{\I{Proportions-by-category}}\label{sec:Observation-ProportionsByCategory}
...
The observation is supplied for a given year and time-step, for selected \ifAgeBased age \else length \fi classes of the population (i.e., for a range of \ifAgeBased ages \else lengths \fi).

\ifAgeBased 
The age range must be ages defined in the partition (i.e., between \commandsub{model}{min\_age} and \commandsub{model}{max\_age} inclusive); 
\else 
...
Proportions-by-category observations can be supplied for any set of categories as a proportion of themselves and any set of additional categories. For example, for a model with the two categories \emph{male} and \emph{female}, observations of the proportions of males in the population at each \ifAgeBased age \else length \fi class might be provided. The subcommand \subcommand{categories} defines the categories for the numerator in the calculation of the proportion, and the subcommand \subcommand{total\_categories} supplies the additional categories to be used in the denominator of the calculation. In addition, each category must have an associated selectivity, defined by \subcommand{selectivities} for the numerator categories and \subcommand{total\_selectivities} for the additional categories used in the denominator.

For example,

{\small{\begin{verbatim}
	categories male
	total_categories female
	selectivities male-selectivity
	total_selectivities female-selectivity
\end{verbatim}}}

defines the proportion of males in each \ifAgeBased age \else length \fi class as a proportion of males and females combined. \CNAME\ then requires that there will be a vector of proportions supplied, with one proportion for each \ifAgeBased age \else length \fi class within the defined range, i.e., if the \ifAgeBased age range was 3 to 10 \else length bins were 21 to 28 \fi, then 8 proportions should be supplied (one proportion for each of the \ifAgeBased age classes \else length bins \fi in the defined range). The expected values will be the expected ratios of male to male $+$ female within each of these age classes, after applying the selectivities at the year and time-step specified.

\CNAME\ calculates the expected values by summing over the \ifAgeBased ages \else lengths \fi (via the range and selectivity).
...
\ifAgeBased
\subsubsection{\I{Tag recaptures by age or length}} \label{sec:Observation-TagRecaptures} \label{sec:Observation-TagRecaptureByAge} \label{sec:Observation-TagRecaptureByLength}
...
\subsubsection{\I{Tag recaptures by length}} \label{sec:Observation-TagRecaptures} \label{sec:Observation-TagRecaptureByLength}
...
\subsubsection{\I{Tag recapture by length for growth}\label{sec:Observation-TagRecaptureByLengthForGrowth}}
...
\ifAgeBased 
\TODO{At this time, there are no process only observations for length based models. When implemented, then this section will need to be modified}

\subsection{\I{Observations that are process observations only} \label{secProcessOnlyObservations}}

Some observations cannot be time step observations. They are only defined as process observations. These are
...
\subsubsection{\I{Process removals by age}\label{sec:removals-by-age}}\label{sec:Observation-ProcessRemovalsByAge}
...
\subsubsection{\I{Process removals by age retained} \label{sec:Observation-ProcessRemovalsByAgeRetained} \label{sec:Observation-ProcessRemovalsByAgeRetainedTotal}}
...
\subsubsection{\I{Process removals by length} \label{sec:removals-by-length} \label{sec:Observation-ProcessRemovalsByLength}}
...
\subsubsection{\I{Process removals by length retained}} \label{sec:Observation-ProcessRemovalsByLengthRetained} \label{sec:Observation-ProcessRemovalsByLengthRetainedTotal}
...
\subsubsection{\I{Proportions migrating} \label{sec:Proportions-migrating}} \label{sec:Observation-ProportionsMigrating}
...
{\small{\begin{verbatim}
	...
	categories male.west+female.west ## Categories to evaluate the proportion for
	...
	table error_values
	## if lognormal these are CV's
	1993 0.25
	end_table
\end{verbatim}}}


\subsubsection{\I{Tag recapture by fishery}\label{sec:Observation-TagRecaptureByFishery}}
...
There is currently only one likelihood available for this observation, the Poisson.
...
\subsection{\I{Likelihoods} \label{sec:Likelihood}}

\CNAME\ has a range of likelihoods for the different types of observations, and specific likelihoods can have different calculation s when applied to different types of observations. The likelihoods are described below. 

\subsection{\I{Likelihoods for composition observations}}
...
where the default value of $\delta$ is $1 \times 10^{-11}$.
...
For the observed proportions at age $O_b$ for bin $b$, with CV $c_b$, and the expected proportions at the same age classes $E_b$, the negative log-likelihood is defined as;
...
and the $c_b$'s are the CVs for each composition bin $b$, and $Z \left(\theta,\delta \right)$ is a robustifying function to prevent division by zero errors, with parameter $\delta>0$.
...
\subsection{Likelihoods for abundance and biomass observations} \label{Obs:biomass}

Abundance and biomass observations are expected as an annual time series in \CNAME, where they select the same categories over that time series. The parameters and inputs needed to use this observation class are: a observation $O_i$, CV $c_i$, catchability coefficient $q$, where $i$ indexed the year. \CNAME\ calculates an expectation $E_i$ and scales it by $q$ before comparing it to $O_i$. This means that the value chosen for $q$ will determine whether the observation is relative ($q\neq 1$) or absolute $q = 1$. Before we describe each of the likelihoods we will discuss the methods available to handle $q$s:

\begin{itemize}
	\item The $q$s can be treated as 'nuisance' parameters. For each set of values of the free parameters, the model uses the values of the $q$s which minimise the objective function. These optimal $q$s are calculated algebraically (see Section~\ref{subsec:nuisance}). If one of the $q$s falls outside the bounds specified by the user, it is set equal to the closest bound. This approach reduces the size of the parameter vector and hence should improve the performance of the estimation method. However, it is not ideal when calculating a sample from the posterior in a Bayesian analysis (except asymptotically, see \cite{Walters_ludwig_94}) and we offer the following alternative;
	\item The $q$s can be treated as ordinary free parameters.
\end{itemize}

For both options, it is necessary to evaluate the contribution of $O_i$ to the negative log likelihood for a given value of $q$. Each observation $O_i$ varies about $qE_i$, which expresses the variability of $O_i$ in terms of its CV $c_i$ (or in one case, its standard deviation si). Here are the likelihoods, which are expressed on the objective-function scale of -log(L):
...
This formulation reflects the distributional assumptions that  $O_i$ has the lognormal distribution, that the mean of $O_i$ is $qE_i$  and the CV of $O_i$ is $c_i$.

\subsubsection{The normal likelihood \index{Normal likelihood} \index{Normal likelihood}}

For observations $O_i$, CV $c_i$, and expected values $qE_i$, the negative log-likelihood is defined as;
...
This reflects the distributional assumptions that  $O_i$ has the normal distribution, that the mean of $O_i$ is $qE_i$  and the CV of $O_i$ is $c_i$.

\subsection{Likelihoods for tag recapture by age and length observations}
\subsubsection{The binomial likelihood \index{Binomial likelihood ! tag-recapture-by-length}}
...
\subsection{\I{Likelihoods for proportions-by-category observations}}
...
\subsection{\I{Data weighting}}

Data weighting can be modified by the use of subcommands for each observation. This can be defined for observations using either \I{process error}, an \I{error value multiplier}, a \I{likelihood multiplier}, or (in the case of tag recaptures) an \I{overdispersion} parameter.  The different methods have the effect of up-weighting or down-weighting the likelihood contribution for a specific observation. 

Process errors (\subcommand{process\_errors}) increase the observation error in the data, and hence of decreasing the relative weight given to the data. The method by which process errors modify the error values depends on the likelihood type (see below).
...
Note that \subcommand{process\_errors} is not allowed for tag recapture observations --- use the dispersion subcommand instead.
...
The \subcommand{free} method treats the catchability parameters like all other parameters, where a prior needs to be assumed and it is estimated in the usual fashion.

The \subcommand{nuisance} method treats the catchability parameters like all other parameters, where a prior needs to be assumed and it is estimated in the usual fashion. We recommend the \subcommand{free} method as the default, with the \subcommand{nuisance} method used for backwards compatibility only.

Note that when estimating any parameter (including both the \subcommand{free} and \subcommand{nuisance} catchabilities), the minimiser uses a value of $10^{-6}$ to determine if an estimate is close to a bound. Some catchability estimates can approach or be lower than this value (especially CPUE catchability values) and consequently may cause the minimiser to return a false minimum. In the case of relative indices of biomass or abundance, we recommend using the work-around by arbitrarily multiplying the observations by some large constant, for example $10^4$. This will have the effect of modifying the value of the catchability by this constant, and reduce the chances that the minimiser returns a false minimum.

\subsubsection{\I{Free catchability (\emph{q}) parameters} \label{subsec:free}}

When the catchability type is \subcommand{free}, estimation is the same as for any other model parameter. See Section~\ref{syntax:Catchability-Free} for additional details on the specific syntax.
...
\subsubsection{\I{Nuisance catchability (\emph{q}) parameters} \label{subsec:nuisance}}
The algorithms \CNAME\ uses to derive nuisance (analytical) catchability coefficients $q$s are described below. See Section~\ref{syntax:Catchability-Nuisance} for additional details on syntax. Note that you can only use nuisance $q$s when the additional prior on $q$ is one of the following:

\begin{itemize}
	\item Uniform 
	\item Uniform-log
	\item Lognormal with observations with a lognormal likelihood
\end{itemize}

And the equations for the nuisance catchability $q$ are summarised in Table~\ref{tab:nus_overview} and described below.

\begin{table}[h!]
	\caption{\textbf{Equations used to calculate nuisance $q$s (*=no analytic solution available).}}\label{tab:nus_overview}
	\begin{tabular}{cccccc}
		Distribution  & Uniform & Uniform-log & Normal & lognormal\\
		\hline
		Normal &  \eqref{EQ:1} & \eqref{EQ:3} & \textbf{*} & \textbf{*} \\
		Lognormal  & \eqref{EQ:4} & \eqref{EQ:8} & \textbf{*} & \eqref{EQ:9} \\
	\end{tabular}
\end{table}
...
First, consider when the ($Oi$) are assumed to be normally distributed and the additional prior is uniform:
...
When the ($O_i$) are assumed to be lognormally distributed, then
...
When the prior for $q$ is specified as either uniform-log or lognormal, then the effects of the prior on the equations are to replace likelihood $L$ by posterior $P$ throughout, to add $-log(\pi(q))$ to the equation for $-log(P)$ and $\partial/\partial q(-log(-\pi(q)))$ to the equation for $\partial/\partial q(-log(P))$

This last term is $0$ for a uniform prior on $q$, $1/q$ for a log-uniform prior, and $\frac{1}{q}\bigg( 1.5 + \frac{log(q) - log(\mu_q)}{\sigma_q^2} \bigg)$ for a lognormal prior, where $\mu_q$ and $c_q$ are the mean and CV of the prior on $q$, respectively, and $\sigma_q = \sqrt{log(1+c_q^2)}$. 

Since the prior is uniform, the equation for $\hat q$ is the same as the maximum likelihood estimation.
...
It is not possible to solve for $\hat q$ with a normal prior, and hence nuisance catchability priors cannot be normal.

An example of the syntax and an equivalent additional prior is:
...
\begin{itemize}
  \item{} Normal likelihood parameterised by CV: Let $E_{i}$ be the fitted value for observation $i$, and $c_i$ be the corresponding CV (adjusted by the process error if applicable). Each simulated observation value $S_i$ is generated as an independent normal deviate with mean $E_i$ and standard deviation $E_i c_i$.
  \item{} Log-normal likelihood: Let $E_i$ be the fitted value for observation $i$ and $c_i$ be the corresponding CV (adjusted by the process error if applicable). Each simulated observation value $S_i$ is generated as an independent lognormal deviate with mean and standard deviation (on the natural scale, not the log-scale) of $E_i$ and $E_i c_i$ respectively. The robustification parameter $\delta$ is ignored.
...
\begin{itemize}
  ...
  \item When simulating observations, the subcommand \subcommand{simulation\_likelihood} to indicate the likelihood to use. In this case, the \subcommand{obs}, \subcommand{error\_value}, and \subcommand{process\_error} are used to determine the appropriate terms to use for the likelihood if they are to be used in a simulation.
\end{itemize}


\subsection{\I{Residuals}}\label{sec:Residuals}

\CNAME\ will print the default residual values (i.e., observed minus expected) when the report type \command{report}\subcommand{.type=observation} is used. For an observation with observed values \textit{O} and expected values \textit{E}, then the residual is simply defined as

\begin{itemize}
	\item $\text{Residual} = O - E$
\end{itemize}

Pearson and normalised residuals can be generated using the \CNAME\ \R\ package. For specific \R\ functions available for use with \CNAME, see Section~\ref{sec:PostProcessing}.

The definitions used in the calculations are

\begin{itemize}
	\item \textit{Pearson residuals} attempt to express the residual relative to the variability of the observation, and are defined as ($O-E)/\text{sd}(O$), where $\text{sd}$ is the standard deviation and is calculated as
	\begin{itemize}
		\item $E \times cv$ for normal, lognormal, robustified lognormal, and normal-log error distributions.
	...
	\begin{itemize}
		...
		\item $(\log(O/E)+0.5\sigma^2)/\sigma$ for lognormal (including robustified lognormal) error distributions,\\ where $\sigma= \sqrt{log(1 + cv^2)}$.
		\item  $\log(O/E)/\sigma$ for normal-log error distributions, again with $\sigma= \sqrt{log(1 + cv^2)}$.
		\item And are otherwise undefined.
	\end{itemize}
\end{itemize}

where $Z(\textit{E},r)$ is the robustifying term for $E$ (the expected value of the observation) to avoid divide by zero errors. This robustifying function is described earlier in the description of likelihoods (Section~\ref{sec:Likelihood}).
