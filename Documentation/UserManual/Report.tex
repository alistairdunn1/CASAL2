...
\CNAME generates a range of outputs, both information and error messages, and reports that summarise the model outputs. The report section is where the reports are defined that are used to summarise and print model output. 

The command and subcommand syntax for the report section is given in Section \ref{syntax:Report}.

The report section specifies the printouts and other output from the model. \CNAME\ does not, in general, produce any model specific output unless specified by a valid \command{report} block.

\subsection{Report command block format}

Reports from \CNAME\ can be defined to print partition and states objects at a particular point in time, observation summaries, estimated and derived parameter values, and objective function values. The most useful report is the default report (see Section~\ref{sec:Report-Default}) which will auto generate many of the reports users require.
...
\subsection{\I{Print the value(s) of an addressable}}\index{Reports ! Addressable}\label{sec:Report-Addressable}

This report prints the value or values of an addressable for each given year or given years and time step. This report is evaluated at the end of the time step in the given year(s). This report will print out in run modes \texttt{-r, -e, -f}.

\ifAgeBased
\subsection{\I{Print the age length and length weight values}}\index{Reports ! Age-length}\label{sec:Report-AgeLength}\label{sec:Report-LengthWeight}
...
\else
\subsection{\I{Print the growth model}}\index{Reports ! Growth-Increment}\label{sec:Report-GrowthIncrement}
...
\subsection{\I{Print the output parameters}}\index{Reports ! Output paramters}\label{sec:Report-OutputParameters}
...
\subsection{\I{Print the result of an estimation}}\index{Reports ! Estimated result}\label{sec:Report-EstimationResult}
...
Tabular reporting is specified using the \texttt{-{}-tabular} argument (\texttt{casal2 -r -{}-tabular -i file\_name}), or its short form \texttt{-t}.

Derived quantities, processes, observations, catchabilities, parameter transformations, and \texttt{estimate\_value} reports are the report types that can be output in this format. For each input row the output will begin with the names of each column followed by a multi-line report ending with the \texttt{*end} syntax.

These tables can be read with \R\ using the \CNAME\ \R\ package function \texttt{extract.tabular()}. An example usage is reading in files of MCMC posterior values of derived quantities, which can then be plotted.

\subsubsection{\I{Tab-separated tabular output (\texttt{-{}-tabular-tsv})}}
\index{Reports ! Tabular ! Tab-separated}\label{sec:TabularTSV}

The \texttt{-{}-tabular-tsv} flag (short form \texttt{-T}) is an extension of the tabular reporting mode that writes data rows using \emph{tab} characters as the column separator rather than spaces. It implies \texttt{-{}-tabular} and does not need to be combined with \texttt{-t}.

The primary motivation is performance when importing large MCMC outputs into \R: tab-separated files can be read with \texttt{data.table::fread} or with \texttt{extract.tabular()} (which detects the \texttt{\{dataframe\_tsv\}} marker automatically and uses an optimised internal parser, typically 5--10$\times$ faster than the space-separated path for large chains).

A typical workflow for an MCMC run is:
{\small{\begin{verbatim}
casal2 -E mpd.log             # obtain the MPD
casal2 -M mpd.log -T > mcmc.log 2> mcmc.err   # MCMC with TSV tabular reports
\end{verbatim}}}

In \R:
{\small{\begin{verbatim}
library(Casal2)
tab <- extract.tabular(file = "mcmc.log")
head(tab$EstimateValues$values)  # data frame, one row per retained sample
\end{verbatim}}}

The output format is identical to standard tabular output except that within each \texttt{\{dataframe\}} block the column-header line and all data rows use tab characters as separators. The report boundaries (\texttt{*label (type)}, \texttt{*end}) and key--value lines (e.g.~\texttt{year: 2025}) remain space-delimited.
