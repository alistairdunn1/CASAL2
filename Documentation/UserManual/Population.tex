...
Given a set of estimated parameter values from a \textit{-e} or a MCMC run, the model can be projected. Projection years are after the model run years, and are defined in the \command{model} command block using the \subcommand{final\_projection\_year} subcommand, i.e., projection years are \subcommand{final\_year + 1} through to \subcommand{final\_projection\_year}.
...
The argument \argument{multiplier} is a constant (which may be set the same for all years or a value given for each year) which is multiplied with the projected value (for all but the multiple values method) after it has been generated. 
