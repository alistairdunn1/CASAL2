...
The ramp rule harvest strategy applies a catch equal to $\hat{U}$ times the biomass index (scaled by \argument{multiplier}). It will update the catch that is applied according to the frequency defined by \argument{year\_delta}. The catch will only be applied if the change is greater than \argument{min\_delta}, and the maximum change that can be applied is equal to \argument{max\_delta} (where a value of zero is interpreted as no maximum). 
