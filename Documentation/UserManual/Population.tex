...
\paragraph[Empirical sampling]{Empirical sampling: Sampling from a range of years, type  \argument{empirical\_sampling}}\index{Projections!Empirical sampling}\label{sec:Project-EmpiricalSampling}
...
\paragraph[Lognormal]{Lognormal: Sampling from a Lognormal distribution, type  \argument{lognormal}}\index{Projections!Lognormal}\label{sec:Project-LogNormal}
...
\paragraph[Lognormal-Empirical]{Lognormal-Empirical: Sampling from a Lognormal distribution where the  variance is estimated from values over a specified year range, type  \argument{lognormal\_empirical} }\index{Projections!Lognormal-Empirical}\label{sec:Project-LogNormalEmpirical} 
...
\paragraph[HarvestStrategyConstantCatch]{Harvest Strategy Constant Catch: specify a constant catch harvest strategy \argument{harvest\_strategy\_constant\_catch}}\index{Projections!HarvestStrategyConstantCatch}\label{sec:Project-HarvestStrategyConstantCatch} 
...
\TODO{add arguments and explanation}

{\small{\begin{verbatim}
			@project Future_ycs
			type      harvest\_strategy\_constant\_catch
...
\paragraph[HarvestStrategyConstantU]{Harvest Strategy Constant U: specify a constant U harvest strategy \argument{harvest\_strategy\_constant\_u}}\index{Projections!HarvestStrategyConstantU}\label{sec:Project-HarvestStrategyConstantU} 
...
{\small{\begin{verbatim}
			@project Future_ycs
			type      harvest\_strategy\_constant\_U
			parameter process[Instantaneous_Mortality].method_Sub_Ant_F
			years     2012:2016
			biomass_index SSB
			u 0.15
...
\paragraph[HarvestStrategyRampU]{Harvest Strategy Constant U: specify a ramp U harvest strategy \argument{harvest\_strategy\_ramp\_u}}\index{Projections!HarvestStrategyRampU}\label{sec:Project-HarvestStrategyRampU} 

This method applies a harvest strategy from a biomass index (i.e., a derived quantity) using a ramp rule. The harvest strategy is a $U$ where the catch is updated according to a ramp rule for the exploitation rate ($U$).

The biomass index and reference index are derived quantities. A derived quantity of \subcommand{type=biomass\_index} is recommended as this will allow the introduction of random error, autocorrelation, and bias into the index when calculating the catch from the exploitation rate.
