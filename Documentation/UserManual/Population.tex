...
The command and subcommand syntax for the population section is given in Section \ref{syntax:Population}.

\subsection{Introduction}

The population section\index{Population section} defines the specification of the population dynamics of a model. It describes the model time and \ifAgeBased age \else length \fi scope, the population processes used (e.g., recruitment, \ifAgeBased ageing \else growth transition \fi, migration, and mortality), the selectivity ogives, and how to set values for their associated parameters, or starting values if they are going to be estimated.
...
Processes can change the partition and, for example, include recruitment, natural mortality, fishing mortality, \ifAgeBased ageing \else growth transition \fi, migration, and maturation. These processes are repeated for each year of the model.
...
\begin{figure}
	...
	\caption{A example sequence for an annual cycle for an age-based model. In length based models, the ageing process would usually be replaced with a growth transition process}\label{Fig:annual}
\end{figure}
...
For most age-based models, the most common type of initialisation phase to define the initial equilibrium age structure is \subcommand{derived}, but \subcommand{iterative} initialisation can also be used. For length-based models the only type available is \subcommand{iterative}. Additional initialisation phases can be included by sequencing other phases one after another.
...
\input{GrowthAge.tex}
\else
\subsection{\I{Growth increments}\label{sec:GrowthIncrement}}
...
where the $cv$ is the coefficient of variation (CV) of the length-at-age relationship. This adjustment is exact for the lognormal distributions, and an approximation for normal distributions if the CV is not large \citep{1388}. 
...
\subsection{\I{Maturity, in models without maturation in the partition}\label{sec:maturity-notinpartition}}
...
\subsubsection[Double normal Stock Synthesis]{\argument{double\_normal\_stock\_synthesis}} \index{Selectivities!Double-normal-stocksynthesis} \label{sec:Selectivity-DoubleNormalStockSynthesis}
...
\subsection{\I{Time-varying parameters}} \label{sec:TimeVarying}
...
\subsection{\I{Projections}} \label{sec:Project}\index{Projections}

Projections are model runs, using a set of parameters, that are used to project the model forward in time to predict a future state. 

\texttt{casal2 -f $n$} 

to project each set of parameter values $n$ times into the future. 

A set of parameter values from the file specified with the argument -i or -I must be supplied. Projections are repeated for each parameter set (i.e., each line of data in the free parameter file) $n$ times. Typically, the MCMC sample output will be used for projections using the output \texttt{samples} file with -i.

\subsubsection{Specifying projections}

Given a set of parameter values from a model in a form suitable as an input parameter file (i.e., in a format suitable for \texttt{-i}, typically generated from an  estimation (\textit{-e}) or a MCMC (\texttt{-m}) run), the model can be projected. Projection years are after the model run years, and are defined in the \command{model} command block using the \subcommand{final\_projection\_year} subcommand, i.e., projection years are \subcommand{final\_year + 1} through to \subcommand{final\_projection\_year}.
...
The \subcommand{type} subcommand gives the method to use to generate new parameter values.

The argument \argument{multiplier} is a constant (which may be set the same for all years or a value given for each year) which is multiplied with the projected value after it has been generated. 
...
\CNAME\ has no default projection properties for parameters that are specified by year, e.g., recruitment multiplier parameters, time-varying parameters, or future catches. For these, projections  must have a \command{project} command block. For example, \CNAME\ will produce errors if run in projection mode without a \command{project} block for the \subcommand{recruitment\_multipliers} parameter being specified.
...
Future catches are also specified in a \command{project} block, one for each fishery (see \ref{sec:Project-Catch} for examples). Here, a fishery is reference in the \subcommand{parameter} subcommand with the \textit{method\_} fragment to identify it,

\subcommand{process[block label].method\_$[$fisheries label$]$},

For a process called \subcommand{Fishing} that has three fisheries defined, it would be \subcommand{process[Fishing].method\_pot} to specify the fishery labelled \textit{pot}.
...
\label{sec:ProjectionMethods}

\subsubsection{\I{Projection method of type constant}}
\index{Projections!Constant}\label{sec:Project-Constant}

A parameter can either be fixed during all projection years or specified individually for each projection year using \subcommand{constant} projection command. This is a deterministic assumption, where the parameter is assumed to be known without error during projection years.
...
{\small{\begin{verbatim}
	...
	years 2012:2016
	values 1.0 2.0 1.0 2.0 0.5 # one value per year. Alternatively, a single value means 
	                             that all years use the same value
\end{verbatim}}}

\subsubsection{\I{Projection method of type multiple values}}
\index{Projections!Multiple Values}\label{sec:Project-MultipleValues}

Users can specify a set of values for each projection year for each row in the \texttt{-i} or \texttt{-I} file using the \subcommand{multiple\_values} projection command. This gives users flexibility in specifying a range of bespoke values during projections and including uncertainty in the projected values. See below for an example on how to configure this.
...
\subsubsection{\I{Projection method of type empirical sampling}}
\index{Projections!Empirical sampling}\label{sec:Project-EmpiricalSampling}

Parameters that have time components associated with them can be sampled uniformly with replacement over a range of years and used as values for the projected years with the \command{empirical\_sampling} projection command. The year range to sample from is between \argument{start\_year} and \argument{final\_year}:
...
Note that if projecting \argument{recruitment\_multipliers} with the Haist parameterisation of recruitment multipliers, the \emph{unstandardised} values will be used. This may cause projections to be other than what was intended unless these (by chance) have a mean of one. Note that \CNAME\ will not warn nor report an error if this is done. \index{Projections!Haist parameterisation}

In this specific case, project the \argument{standardised\_recruitment\_multipliers} instead, as these will have a mean of one (by definition). If using \argument{standardised\_recruitment\_multipliers}, note that \CNAME\ versions before v24.01 may not implement this correctly and a more recent version is recommended. \index{Projections! standardised\_recruitment\_multipliers}
...
\subsubsection{\I{Projection method of type lognormal}}
\index{Projections!lognormal}\label{sec:Project-LogNormal}

The \command{lognormal} projection command draws parameters from a Gaussian (Normal) distribution in log space and exponentiated to result in the lognormal distribution
...
\subsubsection{\I{Projection method of type lognormal-empirical}}
\argument{lognormal\_empirical} \index{Projections!lognormal-empirical}\label{sec:Project-LogNormalEmpirical} 

The \command{lognormal\_empirical} command applies a lognormal draw as in the \argument{lognormal} method above and specifies a year range from which the standard deviation of the distribution is calculated. Then equation~\eqref{eq:lognormal} is used to generate future values with a specified $\mu$ and empirically calculated $\sigma$,
...
\subsubsection{\I{Projecting catch}}
\index{Projections!Catches}\label{sec:Project-Catch}

Specifying catches in a projection may require a slightly different format from the options identified above, as catches are usually specified in a table format. Here the table name (i.e., method) needs to be appended to specify the parameter to project. This uses the syntax \texttt{block\_type[block\_label].method\_fishinglabel}. 

For example, to project the \argument{fishingPot} catches that are in the following table
...
use the parameter label	\argument{process[Fishing].method\_fishingPot}, i.e., 
...
\subsubsection{\I{Projection method of type harvest strategy constant catch}}
\index{Projections!HarvestStrategyConstantCatch}\label{sec:Project-HarvestStrategyConstantCatch} 

The \command{harvest\_strategy\_constant\_catch} command applies a harvest strategy from a biomass index (i.e., a derived quantity). The harvest strategy is a constant catch method where the catch is updated according to a proportional change in the biomass index. 

The biomass index is a derived quantity. A derived quantity of \subcommand{type=biomass\_index} is recommended as this will allow the introduction of random error, autocorrelation, and bias into the index when calculating the catch from the exploitation rate.

The constant catch harvest strategy applies a catch equal to \argument{alpha} times the proportional change in the biomass index (scaled by \argument{multiplier}), where the \argument{biomass\_index} is the value of the derived quantity from the current year less the \argument{biomass\_index\_lag\_years}. It will update the catch that is applied according to the frequency defined by \argument{update\_frequency\_years}. The catch will only be applied if the change is greater than \argument{min\_delta}, and the maximum change that can be applied is equal to \argument{max\_delta} (where a value of zero is interpreted as no maximum). 

The harvest strategy rule is only applied in the years in the projection period from \argument{first\_year}. Before \argument{first\_year} the \argument{current\_catch} is applied.
...
\subsubsection{\I{Projection method of type harvest strategy constant U}} \index{Projections!HarvestStrategyConstantU}\label{sec:Project-HarvestStrategyConstantU} 

The \command{harvest\_strategy\_constant\_u} method applies a harvest strategy from a biomass index (i.e., a derived quantity) and an exploitation rate. The harvest strategy is a constant $U$ method where the catch is updated according to constant exploitation rate ($U$).

The biomass index is a derived quantity. A derived quantity of \subcommand{type=biomass\_index} is recommended as this will allow the introduction of random error, autocorrelation, and bias into the index when calculating the catch from the exploitation rate.

The constant $U$ harvest strategy applies a catch equal to \argument{U} times the biomass index (scaled by \argument{multiplier}), where the \argument{biomass\_index} is the value of the derived quantity from the current year less the \argument{biomass\_index\_lag\_years}. It will update the catch that is applied according to the frequency defined by \argument{update\_frequency\_years}. The catch will only be applied if the change is greater than \argument{min\_delta}, and the maximum change that can be applied is equal to \argument{max\_delta} (where a value of zero is interpreted as no maximum). 
...
\subsubsection{\I{Projection method of type harvest strategy ramp U}}  
\index{Projections!HarvestStrategyRampU} \label{sec:Project-HarvestStrategyRampU} 

The \command{harvest\_strategy\_ramp\_u} method applies a harvest strategy using a ramp rule. The harvest strategy is an exploitation rate $\hat{U}$ where the catch is updated according to a ramp rule using \argument{u} and the \argument{reference\_index} to calculate $\hat{U}$, with this then multiplied by the \argument{biomass\_index} to determine the catch to apply.

The biomass index and reference index are derived quantities. Derived quantities of \subcommand{type=biomass\_index} is recommended as this will allow the introduction of random error, autocorrelation, and bias into the index when calculating the catch from the exploitation rate. The \argument{biomass\_index} and \argument{reference\_index} can be the same derived quantity.

The ramp $U$ harvest strategy calculates the $\hat{U}$ to be applied from a ramp rule defined by a vector of \argument{u}'s and a equal length vector of \argument{reference\_points}. Here, the value of $\hat{U}$ is a linear interpolation of the values of \argument{u} at the inflection points defined by the \argument{reference\_points}. 

Both the \argument{reference\_index} and \argument{biomass\_index} are derived quantities where their value is equal to the value of the current year less the \argument{biomass\_index\_lag\_years}. The \argument{reference\_index} is then divided by its value from the initialisation phase (i.e., is a proportion). Note that if a lognormal distribution is assumed for \argument{reference\_biomass}, the ratio of the initial to current biomass may need to be adjusted for the bias in the expectation. This can be applied using \argument{bias\_adjustment=true}. 

The ramp rule harvest strategy applies a catch equal to $\hat{U}$ times the biomass index (scaled by \argument{multiplier}). It will update the catch that is applied according to the frequency defined by \argument{update\_frequency\_years}. The catch will only be applied if the change is greater than \argument{min\_delta}, and the maximum change that can be applied is equal to \argument{max\_delta} (where a value of zero is interpreted as no maximum). 

In addition, the value of $\hat{U}$ that is used is equal to the lowest value of \argument{u} when the reference index is less than the first value of \argument{reference\_points}, and equal to the highest value of \argument{u} when the reference index is greater than last value of \argument{reference\_points}. Otherwise, $\hat{U}$ is derived using linear interpolation between inflection points of the reference index.
