...
\defType{Data table with label = prey\_selectivities\_by\_year}
\defDefault{No default}
\defNote{See \ref{sec:DataTable} for more details on specifying data tables}

\defSub{table}{The table of data specifying the prey selectivities}
\defType{Data table with label = predator\_selectivities\_by\_year}
\defDefault{No default}
\defNote{See \ref{sec:DataTable} for more details on specifying data tables}
...
\defDefault{No default. e below for example} \hfill \vspace{-\baselineskip}
\begin{verbatim}
	table catches
...
\defDefault{No default. See below for example} \hfill \vspace{-\baselineskip}
\begin{verbatim}
	table method
...
\defSub{table catches}{The table of data specifying the catches for each fishery and year}
\defDefault{No default}
\defValue{A column of catches as biomass or abundance; or a column of exploitation rates of biomass or abundance. Valid column names are \argument{year} and \argument{catches}}
\defLowerBound{0 (inclusive). See below for example} \hfill \vspace{-\baselineskip}
\begin{verbatim}
	table catches
	year Fishery1_label Fishery2_label
	1993 34 34 
...
\defDefault{No default}
\defValue{Valid column names are: \argument{method}, \argument{category}, \argument{selectivity}, \argument{u\_max}, \argument{time\_step}, and \argument{penalty}; and optionally \argument{biomass}, \argument{u}, and \argument{age\_weight\_label}. See below for example} \hfill \vspace{-\baselineskip}
\begin{verbatim}
	table method
	method category selectivity u_max time_step penalty biomass u
	Fishery1_label male fish_sel 0.99 step1 CatchMustBeTaken true false
	Fishery2_label male fish_sel 0.99 step1 CatchMustBeTaken true false
	end_table
\end{verbatim}
...
\defLowerBound{0.0 (inclusive)}
\defUpperBound{1.0 (inclusive)}
\defNote{The proportions must be specified to sum to 1.0. Proportions with value 0 will result in a warning}
...
\defSub{proportions}{The proportion for each category}
\defType{Real number (estimable)}
\defDefault{An equal proportion in each category}
\defLowerBound{0.0 (inclusive)}
\defUpperBound{1.0 (inclusive)}
\defNote{The proportions must be specified to sum to 1.0. Proportions with value 0 will result in a warning}
...
\defValue{A \(n_y \times (n_l\times n_c) + 1\) matrix, where $n_y=$ is the number of years, \(n_a\) are the ages and \(n_c\) are the number of categories. The first column is the year value for that row. See below for an example} \hfill \vspace{-\baselineskip} 
\begin{verbatim}
	table numbers 
	1993 34 34 23 43
	1994 23 34 23 43
...
\defType{Data table with label = numbers}
\defDefault{Can be replaced with 'table proportions' -- see below}
\defValue{A \(n_y \times (n_l\times n_c) + 1\) matrix, where $n_y=$ is the number of years, \(n_a\) are the number of ages ages and \(n_c\) are the number of categories. The first column is the year value for that row. See below for an example} \hfill \vspace{-\baselineskip}
\begin{verbatim}
	table numbers 
	1993 34 34 23 43
	1994 23 34 23 43
...
\defType{Data table with label = proportions}
\defDefault{Can be replaced with table numbers -- see above}
\defValue{A \(n_y \times (n_l\times n_c) + 1\) matrix, where $n_y=$ is the number of years, \(n_a\) are the number of ages and \(n_c\) are the number of categories. The first column is the year value for that row. See below for an example} \hfill \vspace{-\baselineskip}
\begin{verbatim}
	n 200 300 ## need to specify n if you give proportions
	table proportions 
	1993 0.1 0.2 0.7
...
\defDefault{No default. See below for an example} \hfill \vspace{-\baselineskip}
\begin{verbatim}
table n
year 3 4 5 6
