...
\defSub{table \emph{obs}}{The table of data specifying the observed values}
\defType{Data table with label = \subcommand{obs}}
\defDefault{No default}
\defValue{A $n*m$ matrix, where $n=$ the years and $m=$ either the number of ages, lengths, or abundance/biomass observation for each year defined in the model. Each row starts with the year. The table ends with the subcommand \subcommand{end\_table}}
\defNote{See \ref{sec:DataTable} for more details each observation may have custom table labels.}

\defSub{table \emph{error\_values}}{The table of data specifying the observed error values}
\defType{Data table with label = \subcommand{error\_values}}
\defDefault{No default}
\defValue{A $n*m$ matrix, where $n=$ the years and $m=$ either the number of ages, lengths, or abundance/biomass observation for each year defined in the model. Each row starts with the year. The table ends with the subcommand \subcommand{end\_table}.}
