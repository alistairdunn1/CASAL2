...
\subsubsection{Observation of type abundance and type process\_abundance} \commandlabsubarg{observation}{type}{Abundance} \\ \commandlabsubarg{observation}{type}{Process\_Abundance} 
\defRef{sec:Observation-Abundance} \label{syntax:Observation-Abundance}

\defSub{time\_step}{The label of the time step that the observation occurs in}
\defType{String}
\defDefault{No default}

\defSub{time\_step\_proportion \text{\normalfont or} process\_proportion}{The proportion through the mortality block (for time step observations) or the proportion through the process (for process observations) when the observation is evaluated}
\defType{Real number (estimable)}
\defDefault{0.5}
\defLowerBound{0.0 (inclusive)}
\defUpperBound{1.0 (inclusive)}
\defNote{Use either \subcommand{time\_step\_proportion} for time step observations or \subcommand{process\_proportion} for process observations}

\defSub{process}{The label of the process if this is a process observations}
\defType{string}
\defDefault{none}
\defNote{Only valid if this is a process observation}
...
\defDefault{No default}
\defValue{A $n*3$ matrix, where $n=$ the years and a column for year, observation and error value. See below for example.} \hfill\vspace{-\baselineskip}
\begin{verbatim}
table obs 
...
\subsubsection{Observation of type biomass or type processs\_biomass}
\commandlabsubarg{observation}{type}{Biomass} \\ 
\commandlabsubarg{observation}{type}{Process\_Biomass} 
...
\defSub{process\_proportion}{The proportion through the process when the observation is evaluated}
\defType{Real number (estimable)}
\defDefault{0.5}
\defLowerBound{0.0 (inclusive)}
\defUpperBound{1.0 (inclusive)}
...
\defType{Data table with label = obs}
\defDefault{No default}
\defValue{A $n\times m$ matrix, where $n=$ the years and $m$ is categories \(\times\) length bins. See below for example.} \hfill\vspace{-\baselineskip}
\begin{verbatim}
table obs 
1993 0.1 0.2 0.3
1994 0.1 0.2 0.3
...
\defDefault{No default}
\defValue{Can be specified two ways either as a $n\times 1$ matrix with an error value for each year. Or a $n\times m$ matrix, where $n=$ the years and $m$ is categories \(\times\) length bins. See below for example.} \hfill\vspace{-\baselineskip}
\begin{verbatim}
table error_values 
1993 234
1994 343
...
\commandlabsubarg{observation}{type}{Process\_Proportions\_At\_Age} 
...
\commandlabsubarg{observation}{type}{Process\_Proportions\_At\_Length} 
...
\commandlabsubarg{observation}{type}{Process\_Proportions\_By\_Category} 
...
\defSub{categories}{The categories in the numerator}
\defType{Vector of strings}
\defDefault{No default}
\defNote{The categories is the list of categories in the numerator but not in the denominator, where the categories in the denominator include the categories in both \subcommand{categories} and \subcommand{total\_categories}}

\defSub{selectivities}{The labels of the selectivities for the categories}
\defType{Vector of strings}
\defDefault{true}

\defSub{total\_categories}{The total categories}
\defType{Vector of strings}
\defDefault{No default}
\defNote{The total categories is the list of categories in the denominator but not in the numerator, i.e., categories in the denominator include the categories in both \subcommand{categories} and \subcommand{total\_categories} It is an error to specify a category in both \subcommand{categories} and \subcommand{total\_categories}}

\defSub{total\_selectivities}{The selectivities for the total categories}
\defType{Vector of strings}
\defDefault{No default}
\defNote{The total selectivities is the list of selectivities for those categories in \subcommand{total\_categories}}

\subsubsection{Observation of type proportions migrating}
\commandlabsubarg{observation}{type}{Proportions\_Migrating} 
% \commandlabsubarg{observation}{type}{Process\_Proportions\_Migrating} \\
\defRef{sec:Observation-ProportionsMigrating}
\label{syntax:Observation-ProportionsMigrating}

Note: This observation is exclusively associated with the process type \subcommand{transition\_category}, and will produce an error when associated with any other process type.
...
\defSub{process\_proportion}{The proportion through the transition category when the observation is evaluated}
\defType{Real number (estimable)}
\defDefault{0.5}
\defLowerBound{0.0 (inclusive)}
\defUpperBound{1.0 (inclusive)}
...
\defSub{tagged\_categories}{The tagged categories that we want to generate recaptures for. Categories need to be space separated. Do not use the '+' category syntax.}
\defType{Vector of strings}
\defDefault{No default}
...
\defDefault{No default}
\defValue{A $n\times 2$ matrix, where $n=$ the years and the first column specifies the year and the second column specifies the observed tag recaptures.} \hfill\vspace{-\baselineskip}
\begin{verbatim}
table recaptured
2000 10120
2001 90123
...
\defSub{time\_step\_proportion}{The proportion through the mortality block when the observation is evaluated}
\defType{Real number (estimable)}
\defDefault{0.5}
\defLowerBound{0.0 (inclusive)}
\defUpperBound{1.0 (inclusive)}
...
\defSub{intervals}{The number of quantiles to evaluate the lengths in the age-length relationship}
\defType{Non-negative integer}
\defDefault{5}
