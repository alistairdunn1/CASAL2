...
\defValue{A \(n_y \times (n_l\times n_c) + 1\) matrix, where $n_y=$ is the number of years, \(n_l\) are the number of length bins and \(n_c\) are the number of categories. The first column is the year value for that row. See below for an example}
