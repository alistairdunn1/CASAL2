...
\else
\item growth increments\index{Growth increments} (Section~\ref{sec:GrowthIncrement}),
\fi
...
The year offset \subcommand{ssb\_offset} between spawning and subsequent recruitment to the partition needs to be specified. 
...
For example, to specify a constant recruitment process where individuals are added to the male and female immature categories in length bin 1 in equal proportion (\texttt{proportions} = 0.5), and the number to add is $R_0=5 \times 10^5$, the syntax is
...
Recruitment refers to a cohort recruiting into the partition and may differ from the spawning year. See above for information about \subcommand{ssb\_offset}.
...
The subcommand \subcommand{ssb\_offset} has to be manually specified.

\paragraph*{YCS ($YCS_y$)}

The $YCS$ parameter is reference by the recruited year. The recruited year is the year when a year class or age-cohort enter the partition. The recruited year differs from the spawning event year defined in Equation~\eqref{eq:year_class}. This is a shift away from CASALs terminology which used \subcommand{ycs\_year} and is equivalent to the spawning event year. Standardisation years are also now expressed as recruited years. This will differ from \CNAME\ versions from before August 2022 and for CASAL models. From August 2022 the commands \subcommand{ycs\_values}, \subcommand{ycs\_years}, and \subcommand{standardised\_ycs\_years} were deprecated. These were replaced with \subcommand{recruitment\_multipliers} and \subcommand{standardise\_years}.
...
There are several types of mortality processes available in \CNAME length based models, plus the tag release processes that can also cause mortality (See Tagging Section~\ref{sec:Process-Tagging}):
