...
\defSub{biomass\_index\_scalar}{The biomass value re-scaling parameter}
\defType{Real number}
\defDefault{1.0}
...
\defSub{reference\_index}{The biomass for calculating the status relative to the reference points (i.e., a derived quantity label)}
\defType{String}
\defDefault{No default}
...
\defSub{multiplier}{Multiplier that is applied to the calculated catch value under the harvest strategy rule}
\defType{Vector of real numbers (estimable)}
\defDefault{1.0}
\defLowerBound{0.0 (exclusive)}

\defSub{first\_year}{The first year in which to apply the harvest strategy rule}
\defType{Non-negative integer}
\defDefault{0}

\defSub{b0\_initialisation\_phase}{The initialisation phase label that the initial biomass is from}
\defType{String}
\defDefault{No default}

\defSub{bias\_adjustment}{Indicator whether to adjust for the mean bias in the calculation of the reference biomass}
\defType{Boolean}
\defDefault{false}
