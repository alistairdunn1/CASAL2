...
\commandlabsubarg{additional\_prior}{type}{AR1}

\defSub{parameter}{The name of the parameter for the additional prior}
\defSub{rho}{The AR1 autocorrelation parameter ($\rho$)}
\defSub{sigma}{The innovation standard deviation ($\sigma$)}
\defSub{mean}{The process mean ($\mu$)}
\defSub{multiplier}{Multiply the AR1 penalty by this factor}
\defSub{log\_scale}{Should the AR1 be applied in log space?}

\commandlabsubarg{additional\_prior}{type}{Normal}

\defSub{parameter}{The name of the parameter for the additional prior}
\defSub{mu}{The normal prior mean (mu) parameter}
\defSub{cv}{The normal CV parameter}
...
\defSub{compatibility\_option}{Compatibility option: either casal2 (the default) or casal to use the (less accurate) cumulative normal function from CASAL}
\defSub{distribution}{The assumed distribution for the growth curve}
\defSub{length\_weight}{The label from an associated length-weight block}
...
\defSub{retain\_chain\_history}{Retain full chain history in memory}
...
\commandlabsubarg{observation}{type}{Process\_Abundance}
...
\defSub{years}{The years for which there are observations}
\defSub{selectivities}{The labels of the selectivities}
\defSub{total\_categories}{Categories in the denominator}
...
\defSub{categories}{The categories}
\defSub{fishing\_mortality}{The fishing mortality, $F$, to apply}
...
\defSub{ycs\_values}{This subcommand has been deprecated. See Section~\ref{sec:DefinitionYCS} for more information}
\defSub{ycs\_years}{This subcommand has been deprecated. See Section~\ref{sec:DefinitionYCS} for more information}
\defSub{standardise\_ycs\_years}{This subcommand has been deprecated. See Section~\ref{sec:DefinitionYCS} for more information}
...
\defSub{year\_delta}{The number of years between updates}
\defSub{biomass\_index}{The biomass index for calculating the changes in catch}
...
\defSub{u}{The exploitation rate to apply ($U$)}
\defSub{min\_delta}{The minimum difference (proportion) in catch required before it is updated}
\defSub{max\_delta}{The maximum difference (proportion) in catch that can be applied}
\defSub{year\_delta}{The number of years between updates}
\defSub{biomass\_index}{The biomass index for calculating the changes in $U$}
\defSub{biomass\_index\_scalar}{A multiplicative scalar on the biomass index used for calculating the changes in $U$}
\defSub{biomass\_index\_lag\_years}{The lag (years) of the derived\_quantity that is used for the calculation of the catch}
\defSub{current\_catch}{The current catch to apply at the start of the projections (applied until first\_year)}
\defSub{multiplier}{Multiplier that is applied to the projected value}
\defSub{first\_year}{The first year in which to consider an update using the harvest strategy rule}
...
\defSub{bias\_adjustment}{Whether to apply the bias adjustment for the lognormal derived quantity in the calculation of the ratio of the initial and current reference index}
\defSub{min\_delta}{The minimum difference (proportion) in catch required before it is updated}
\defSub{max\_delta}{The maximum difference (proportion) in catch that can be applied}
\defSub{year\_delta}{The number of years between updates}
\defSub{biomass\_index}{The biomass index for calculating the changes in $U$}
...
\defSub{multiplier}{Multiplier that is applied to the projected value}
\defSub{first\_year}{The first year in which to consider an update using the harvest strategy rule}
\defSub{b0\_initialisation\_phase}{The phase that defines the initial biomass (e.g., B0)}
...
\defSub{ato95}{The age or length between \(50\%\) and \(95\%\) in the selectivity}
\defSub{alpha}{The maximum value of the selectivity }
\defSub{beta}{The minimum age/length for which the selectivity applies}
