...
\documentclass[a4paper,11pt,twoside,pdftex,fleqn]{article}
...
\usepackage[font=bf,format=hang,labelsep=colon,justification=raggedright,singlelinecheck=false,compatibility=false]{caption}
...
\usepackage[bookmarks=true,bookmarksopen=false,bookmarksnumbered=true,
    ...
    pdfsubject={Casal2 user manual for age-based models, Casal2 Steering Group Technical Report},
    pdftex]{hyperref} 
\hypersetup{
...
\newcommand{\defCom}[2]{\texttt{@#1\index{Command ! #1}} \hspace{0.5cm} {#2}}
% Define a command with a label
\newcommand{\defComLab}[2]{\texttt{@#1\ \emph{label}\index{Command ! #1}} \hspace{0.5cm} {#2}}
% Define a subcommand
\newcommand{\defSub}[2]{\texttt{#1} \hspace{0.5cm} #2 \\*}% Define a command with an argument
\newcommand{\defComArg}[3]{\texttt{@#1\ \emph{#2}\index{Command ! #1}} \hspace{0.5cm} {#3}}
...
\newcommand{\emaillink}{\href{mailto:"Casal2 Development Team"<casal2@zaita.com>?subject=Casal2:}{\texttt{Casal2 Development Team}}}
\newcommand{\github}{\url{https://github.com/Casal2}}
% \newcommand{\authorlink}{\href{mailto:"Casal2 Development Team"<casal2@niwa.co.nz>?subject=Casal2:}{authors}}
% hyper ref for email
\newcommand{\Organisation}{Casal2 Steering Group.}
\newcommand{\ManualRef}{\authors\ (\DocYear). \CNAME\ user manual for age-based models, \VER. \Organisation\ \emph{Technical Report}. \ref{TotPages} p. (Using source code from \SourceRepos)} % full document reference
...
\begin{titlepage}
  \thispagestyle{empty} % no header/footer/page number on this page
	\begin{center}
		...
		~\vfill
		\Large Casal2 Steering Group Technical Report \\% Document Date
