...
The types of additional priors available include smoothing and averaging of vector parameters, a uniform log or lognormal prior on a single parameter, and a prior on the sum of a list of parameters. The additional priors are:

\begin{itemize}
	\item \texttt{vector\_smoothing}\label{sec:AdditionalPrior-VectorSmoothing}\index{Additional prior!Vector smoothing}
	...
	\index{Additional prior!Vector average}

	This prior can be applied to a vector parameter, $v$, with the value of the prior $P$ being one of three methods: 
	\begin{enumerate} 
	...
	\item\texttt{lognormal} with mean $\mu$ and c.v. $c$\label{sec:AdditionalPrior-LogNormal}\index{Additional prior!lognormal}
	
	Apply a lognormal prior to a single parameter,

	\begin{equation}
	- \log \left(\pi \left(p \right) \right) = \log \left( p \right) + 0.5 \left( \frac{\log \left( p / \mu \right)}{s} + \frac{s}{2} \right)^2
	\end{equation}

	\item\texttt{uniform}\label{sec:AdditionalPrior-UniformLog}
	\index{Additional prior!Uniform}
	
	Apply a uniform prior to a single parameter,
	
	\begin{equation}
		- \log \left(\pi \left(p \right) \right) = 0
	\end{equation}
	
	\item\texttt{uniform\_log}\label{sec:AdditionalPrior-Uniform}
	\index{Additional prior!Uniform log}

	Apply a uniform\_log prior to a single parameter,

	\begin{equation}
		- \log \left(\pi \left(p \right) \right) = \log \left( p \right)
	\end{equation}

	\item\texttt{element\_difference}\label{sec:AdditionalPrior-ElementDifference}\index{Additional prior!Element difference}
	
	Apply a penalty to encourage two vectors of equal length to be similar,

	\begin{equation}
	- \log \left(\pi \left(p_1,p_2 \right) \right) = \sum_{i = 1}^n \left( p_{1,i} - p_{2,i} \right)^2
	\end{equation}

	\item\texttt{Beta}\label{sec:AdditionalPrior-Beta}\index{Additional prior!Beta}

	Apply a Beta prior to a single parameter, \index{Beta additional prior}\index{Additional.  Priors ! Beta} with mean $\mu$ and standard deviation $\sigma$, and range parameters $A$ and $B$, for parameter value = $p$
	...
	\item\texttt{Sum}\label{sec:AdditionalPrior-Sum}\index{Additional prior!Sum}
	
	Apply a prior on the sum of a list of parameters, using either a normal or lognormal distribution.
