...
For very long MCMC runs with many estimated parameters, the retained chain history can consume significant memory. By default, the complete chain history is retained in memory. To reduce memory usage during long MCMC runs, set \commandsub{mcmc}{retain\_chain\_history} to \texttt{false}. When disabled, only the most recent chain state is retained in memory, which is sufficient for reporting and convergence, while still allowing full diagnostics and output of posterior samples. This is particularly useful for MCMCs with thousands of iterations and many parameters. Note that unit tests and some diagnostic tools may rely on full chain history, so this option should only be disabled for production MCMC runs.
...
When some of these priors are parameterised in terms of mean, CV, and standard deviation, these refer to the parameters of the distribution before the bounds are applied. The moments of the prior after the bounds are applied may differ.
...
The lognormal distribution with mean $\mu$ and CV $c$
...
The normal-log distribution  with mean $\mu$ and CV $c$
...
The types of additional priors available include smoothing and averaging of vector parameters, an AR1 process prior for sequential parameters, a uniform log, normal, or lognormal prior on a single parameter, and a prior on the sum of a list of parameters. The additional priors are described below.
...
\subsubsection{Normal}\label{sec:AdditionalPrior-Normal}\index{Additional prior!normal}

Apply a \subcommand{normal} additional prior with mean $\mu$ and CV $c$ to a single parameter,
...
Apply a \subcommand{lognormal} additional prior with mean $\mu$ and CV $c$ to a single parameter,
...
\subsubsection{AR1}\label{sec:AdditionalPrior-AR1}\index{Additional prior!AR1}

Apply an \subcommand{ar1} additional prior to a sequential vector (or map) parameter to encourage temporal autocorrelation around a process mean $\mu$.

For sequence values $x_1,\dots,x_n$ the AR1 residual in year $t$ is

\begin{equation}
e_t = (x_t - \mu) - \rho(x_{t-1} - \mu), \quad t = 2,\dots,n
\end{equation}

and the objective-function contribution is

\begin{equation}
- \log \left(\pi \left(\mathbf{x} \right) \right) = 0.5\times \text{multiplier} \times \sum_{t=2}^{n}\frac{e_t^2}{\sigma^2}
\end{equation}

where $\rho$ is the AR1 autocorrelation parameter and $\sigma$ is the innovation standard deviation. If \subcommand{log\_scale true}, the AR1 prior is applied to $\log(x_t)$ values.
...
And the lognormal distribution has mean $\mu$ and CV $c$
...
\subsubsection{Transformations with the Jacobian}
...
In equation~\ref{eq:jacobian} the term $p_X(f^{-1}(y)) = p_X(X)$ and in a Bayesian context is the prior of the untransformed variable/parameter. \CNAME\ defines the objective function as the negative log-likelihood. This means \(\begin{vmatrix} \frac{\partial}{\partial y} f^{-1}(y) \end{vmatrix}\) needs to be the negative log value, as it is currently defined as an adjustment to the density.

\label{subsec:Transformation-types}
\subsubsection{The \argument{log} transformation}\label{sec:Transformation-Log}

The natural logarithm transformation has one parameter that is the log of its value. It has the Jacobian defined and the transformation has estimable parameter \argument{log\_parameter}. 
...
The negative log Jacobian is: \\
$-\log(\text{Jacobian}) = \log \begin{vmatrix} \frac{\partial}{\partial y}  \exp(y) \end{vmatrix} = \log \begin{vmatrix} \exp(y) \end{vmatrix} = \log(x)$
...
\subsubsection{The \subcommand{inverse} transformation} \label{sec:Transformation-Inverse}

The inverse transformation has one parameter that is the inverse of its value. It has the Jacobian defined and the transformation has the estimable parameter \argument{inverse\_parameter}.
...
The negative log Jacobian is: \\
$-\log(\text{Jacobian}) = \log \begin{vmatrix} \frac{\partial}{\partial y} \frac{1}{y} \end{vmatrix} = \log \begin{vmatrix} y^{-2}\end{vmatrix} = -2\log(y)$
...
\subsubsection{The \subcommand{difference} transformation} \label{sec:Transformation-Difference}

The difference transformation has two parameters $X_1$ and $X_2$ that are transformed to $X_1$ and $X_1 - d$, where $d$ is the difference between the original parameters. The Jacobian is defined and the transformation has estimable parameter \argument{difference\_parameter}\\
...
\subsubsection{The \subcommand{average\_difference} transformation}  \label{sec:Transformation-AverageDifference}

The average-difference transformation has two parameters $X_1$ and $X_2$ that are transformed to $Y_1$ and $Y_2$, where $Y_1$ is the average of the original parameters and $Y_2$ is the difference between the mean and each parameter. The Jacobian is defined and the transformation has estimable parameters \argument{average\_parameter} and \argument{difference\_parameter}.
...
\subsubsection{The \subcommand{log\_sum} transformation} \label{sec:Transformation-LogSum}

The log-sum transformation has two parameters $X_1$ and $X_2$ that are transformed to $Y_1$ and $Y_2$, where $Y_1$ is the natural logarithm of the sum of $X_1$ and $X_2$. $Y_2$ describes the proportion of the sum with respect to $X_1$.

The Jacobian is defined and the transformation has estimable parameters \argument{log\_total\_parameter} and \argument{total\_proportion\_parameter}.
...
$X_2 =exp(Y_1)(1 - Y_2)$
 ...
 The negative log Jacobian is: \\
 $-\log(\text{Jacobian}) = -\log(2) + 2 \log(\text{total\_parameter})$
...
\subsubsection{The \subcommand{orthogonal} transformation} \label{sec:Transformation-Orthogonal}

The orthogonal transformation has two parameters $X_1$ and $X_2$ that are transformed to $Y_1$ and $Y_2$, where $Y_1$ is the multiplication of $X_1$ and $X_2$. $Y_2$ is the division of $X_1$ and $X_2$.

The Jacobian is defined and the transformation has estimable parameters \argument{product\_parameter} and \argument{quotient\_parameter}.
...
The negative log Jacobian is: \\
$-\log(\text{Jacobian}) = \begin{vmatrix} \frac{\partial}{\partial y} f^{-1}(y) \end{vmatrix} = 2Y_2$
...
\subsubsection{The \subcommand{logistic} transformation}  \label{sec:Transformation-Logistic}

logistic transformation\\

The logistic transformation is the logit transformation of its value. It has parameter $X$ that is transformed to $\text{logit}(X)$.

The Jacobian is defined and the transformation has estimable parameter \argument{logistic\_parameter}.
...
The negative log Jacobian is: \\
$-\log(\text{Jacobian}) = \begin{vmatrix} \frac{\partial}{\partial y}   lb +(ub - lb)  logit^{-1}(y) \end{vmatrix} =  (ub - lb)logit^{-1}(y)\left(1 - logit^{-1}(y)\right)$
...
{\small{\begin{verbatim}
	...
	lower_bound -1000 # theoretically -Inf. Note use +/-20 for parameters bounded on 0-1
	upper_bound 1000 # theoretically Inf
\end{verbatim}}}
	
\subsubsection{The \subcommand{SumToOne} transformation} \label{sec:Transformation-SumToOne}

The sum to one transformation forced a sum-to-one constraint on the values. Given two parameters $X_1$ and $X_2$ that have the constraint $\sum_{i = 1}^2X_i$, then given $X_1$, $X_2 = 1 - X_1$

The Jacobian for the sum-to-one transformation is not defined, and therefore the prior can only be applied to the transformed values. The transformation has estimable parameter \argument{total\_parameter}.
...
\subsubsection{The \subcommand{simplex} transformation} \label{sec:Transformation-Simplex}

% useful reference for the simplex https://mc-stan.org/docs/2_27/reference-manual/simplex-transform-section.html#fn15

The simplex transformation transforms a vector of parameters $\mathbf{X} = (X_1, \dots, X_n)$ which can have either has the constraint $\sum_{i = 1}^n X_i = 1$ or $\sum_{i = 1}^n X_i = n$. It has the Jacobian defined and the transformation has estimable parameter \argument{simplex}).

It translates the vector $\mathbf{X}$ to a new vector parameter \(\mathbf{Y} = (Y_1, \dots, Y_{n - 1})\) which has unconstrained parameter space, i.e., \(Y_i \in (-\infty, \infty)\). 

This transformation follows the implementation in stan \citep{STAN}, where an intermediate variable \(Z_i\) is used. The transformation going from $\mathbf{X}$ to $\mathbf{Y}$ is as follows:\\
...
The negative log Jacobian is: \\
\begin{align*}
-\log(\text{Jacobian}) = |det \ J| = \prod_{i = 1}^{n - 1} Z_i \left(1 - Z_i\right) \left(1 - \sum_{j = 1}^{i - 1}X_j\right)
\end{align*}
...
\subsubsection{The \subcommand{sqrt} transformation} \label{sec:Transformation-Sqrt}

The square root transformation is the square root of its value. The Jacobian is defined and the transformation has estimable parameter \argument{sqrt\_parameter}.
...
The negative log Jacobian is: \\
$-\log(\text{Jacobian}) = -\log{(1/(2\sqrt{x}))}$
...
\subsubsection{Using transformations}
