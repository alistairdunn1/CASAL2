...
class Archiver:
    ...
    def start(self, skip_building="false"):
        ...
        if skip_building != "true":
            ...
            if (
                subprocess.call(
                    self.do_build_ + " thirdparty > third_party.log 2>&1", shell=True
                )
                != EX_OK
            ):
                return Globals.PrintError(
                    "Failed to build the third party libraries. Please check third_party.log for the error"
                )
            ...
            if (
                subprocess.call(
                    self.do_build_ + " library release > release_build.log 2>&1",
                    shell=True,
                )
                != EX_OK
            ):
                return Globals.PrintError(
                    "Failed to build release library. Please check release_build.log for the error"
                )
            ...
            if (
                subprocess.call(
                    self.do_build_ + " library adolc > release_adolc_build.log 2>&1",
                    shell=True,
                )
                != EX_OK
            ):
                return Globals.PrintError(
                    "Failed to build release library. Please check release_adolc_build.log for the error"
                )
            ...
            if (
                subprocess.call(
                    self.do_build_
                    + " library betadiff > release_betadiff_build.log 2>&1",
                    shell=True,
                )
                != EX_OK
            ):
                return Globals.PrintError(
                    "Failed to build release library. Please check release_betadiff_build.log for the error"
                )
            ...
            if (
                subprocess.call(
                    self.do_build_ + " library test > unit_test_build.log 2>&1",
                    shell=True,
                )
                != EX_OK
            ):
                return Globals.PrintError(
                    "Failed to build release library. Please check unit_test_build.log for the error"
                )
            ...
            if (
                subprocess.call(
                    self.do_build_ + " frontend > front_end_build.log 2>&1", shell=True
                )
                != EX_OK
            ):
                return Globals.PrintError(
                    "Failed to build the front end binary. Please check front_end_build.log for error"
                )
        ...
        binary_path = self.output_directory_ = (
            os.path.normpath(os.getcwd())
            + "/bin/"
            + Globals.operating_system_
            + "_"
            + Globals.compiler_
        )
        ...
        subprocess.call(
            "cp " + binary_path + "/frontend/" + binary_name + " Casal2/" + binary_name,
            shell=True,
        )
        subprocess.call(
            "cp "
        ...
        subprocess.call(
            "cp ../Documentation/UserManual/CASAL2_Age.pdf Casal2/Casal2_Age.pdf",
            shell=True,
        )
        subprocess.call(
            "cp ../Documentation/UserManual/CASAL2_Length.pdf Casal2/Casal2_Length.pdf",
            shell=True,
        )
        subprocess.call(
            "cp ../Documentation/ContributorsGuide/ContributorsGuide.pdf Casal2/Casal2_Contributors_Guide.pdf",
            shell=True,
        )
        subprocess.call(
            "cp ../Documentation/UserManual/CASAL2.xml Casal2/CASAL2.xml", shell=True
        )
        subprocess.call(
            "cp ../Documentation/UserManual/CASAL2-dark.xml Casal2/CASAL2-dark.xml",
            shell=True,
        )
        subprocess.call(
            "cp ../Documentation/UserManual/Notepad++_syntax_highlighter.readme Casal2/Notepad++_syntax_highlighter.readme",
            shell=True,
        )
        subprocess.call(
            "cp ../Documentation/UserManual/CASAL2.syn Casal2/CASAL2.syn", shell=True
        )
        subprocess.call(
            "cp ../Documentation/UserManual/TextPad_syntax_highlighter.readme Casal2/TextPad_syntax_highlighter.readme",
            shell=True,
        )
        print("-- Manual copied")
        ...
        command1 = "cp ../R-libraries/Casal2_*.tar.gz Casal2/R-Libraries/."
        print("-- " + command1)
        ...
        if Globals.operating_system_ == "windows":
            ...
            subprocess.call(
                "mv Casal2.tar.gz " + output_directory + "Casal2.tar.gz", shell=True
            )
