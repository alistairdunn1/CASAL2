...
for (i in 1:length(dash_e_folders)) {
  this_mpd <- tryCatch(extract.mpd(path = file.path(test_dir, dash_e_folders[i]), file = "estimate_betadiff.log"), error = function(e) {
    e
  }, warning = function(w) {
    w
  })
...
for (i in 1:length(dash_r_folders)) {
  this_mpd <- tryCatch(extract.mpd(path = file.path(test_dir, dash_r_folders[i]), file = "run.log"), error = function(e) {
    e
  }, warning = function(w) {
    w
  })
...
for (i in 1:length(dash_i_folders)) {
  this_mpd <- tryCatch(extract.mpd(path = file.path(test_dir, dash_i_folders[i]), file = "run.log"), error = function(e) {
    e
  }, warning = function(w) {
    w
  })
...
for (i in 1:length(dash_m_folders)) {
  this_mcmc <- tryCatch(extract.mcmc(path = file.path(test_dir, dash_m_folders[i]), samples.file = "samples.1", objectives.file = "objectives.1"), error = function(e) {
    e
  }, warning = function(w) {
    w
  })
...
#############################################
## Test extract.tabular with --tabular-tsv MCMC output (SBW_2022 short chain)
## This file is produced by: casal2 -M mpd_tsv.log -T -c config_mcmc_tsv.csl2
#############################################
this_tab_tsv <- tryCatch(
  extract.tabular(path = file.path(test_dir, "SBW_2022"), file = "mcmc_tsv.log"),
  error = function(e) e,
  warning = function(w) w
)
if (inherits(this_tab_tsv, "error") | inherits(this_tab_tsv, "warning")) {
  stop("Could not read tabular-tsv MCMC output (mcmc_tsv.log) for TestModel SBW_2022")
}

## Basic structural check: EstimateValues should be present with data rows
if (!("EstimateValues" %in% names(this_tab_tsv))) {
  stop("tabular-tsv MCMC output for SBW_2022 is missing the 'EstimateValues' report")
}
if (!is.data.frame(this_tab_tsv$EstimateValues$values) || nrow(this_tab_tsv$EstimateValues$values) == 0L) {
  stop("tabular-tsv MCMC 'EstimateValues' report has no rows for SBW_2022")
}
...
for (i in 2:length(all_dirs)) {
  ...
  if (length(all_csl) >= 1) {
    for (j in 1:length(all_csl)) {
      this_file <- tryCatch(extract.csl2.file(path = all_dirs[i], file = all_csl[j], quiet = T), error = function(e) {
        e
      }, warning = function(w) {
        w
      })
