...
"transform.comparisons" <- function(Data) {
  ...
  if (n_categories > 2) {
    stop(paste0("This function can currently only deal with an age composition with 2 unique categories, but we found ", n_categories, " please check you have selected the correct observation to weight"))
  }
  ...
  if (n_categories == 1) {
    if (n_bins == 1) {
      ...
      colnames(adjusted_error) <- paste0(unique(Data[, "category"]), "[", bins, "]")
      if ("normalised_residuals" %in% colnames(Data)) {
        colnames(norm_resid) <- paste0(unique(Data[, "category"]), "[", bins, "]")
      }
      if ("pearsons_residuals" %in% colnames(Data)) {
        colnames(pear_resid) <- paste0(unique(Data[, "category"]), "[", bins, "]")
      }
    }
  } else {
    if (n_bins == 1) {
      ...
      colnames(adjusted_error) <- c(paste0(unique(Data[, "category"])[1], "[", bins, "]"), paste0(unique(Data[, "category"])[2], "[", bins, "]"))
      if ("normalised_residuals" %in% colnames(Data)) {
        colnames(norm_resid) <- c(paste0(unique(Data[, "category"])[1], "[", bins, "]"), paste0(unique(Data[, "category"])[2], "[", bins, "]"))
      }
      if ("pearsons_residuals" %in% colnames(Data)) {
        colnames(pear_resid) <- c(paste0(unique(Data[, "category"])[1], "[", bins, "]"), paste0(unique(Data[, "category"])[2], "[", bins, "]"))
      }
    }
  }
