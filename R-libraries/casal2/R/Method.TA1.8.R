...
Method.TA1.8 <- function(model, observation_labels, plot.it = F, xlim = NULL, ylim = NULL) {
  ...
  multiple <- length(observation_labels) > 1

  for (i in 1:length(observation_labels)) {
    ## check report label exists
    if (!observation_labels[i] %in% names(model)) {
      stop(paste0("The report label '", observation_labels[i], "' was not found. The report labels available are: ", paste(names(model), collapse = ", ")))
    }
    ...
    if (this_report$type != "observation") {
      stop(paste0("The report label '", observation_labels[i], "' is not an observation report, it is a '", this_report$type, "' report. Please check that the correct observation label was specified"))
    }

    if (this_report$likelihood != "multinomial") {
      stop(paste0("This function works only for observations that have multinomial distribution. The observation '", observation_labels[i], "' does not. Please check the observation definition"))
    }
    ...
    if (n_categories > 2) {
      stop(paste0("This function can parse age composition data with 2 unique categories only, but ", n_categories, " were parsed. Please check that the correct observation to weight was specified."))
    }
    ...
    if (n_categories == 1) {
      colnames(obs) <- paste0(unique(Data[, "category"]), "[", bins, "]")
      colnames(fit) <- paste0(unique(Data[, "category"]), "[", bins, "]")
      colnames(err) <- paste0(unique(Data[, "category"]), "[", bins, "]")
    } else {
      colnames(obs) <- c(paste0(unique(Data[, "category"])[1], "[", bins, "]"), paste0(unique(Data[, "category"])[2], "[", bins, "]"))
      colnames(fit) <- c(paste0(unique(Data[, "category"])[1], "[", bins, "]"), paste0(unique(Data[, "category"])[2], "[", bins, "]"))
      colnames(err) <- c(paste0(unique(Data[, "category"])[1], "[", bins, "]"), paste0(unique(Data[, "category"])[2], "[", bins, "]"))
    }
  ...
  for (i in 1:Ndat) {
    ...
    Nassumed <- c(Nassumed, apply(as.matrix(thisfit$error.value), 1, mean))
