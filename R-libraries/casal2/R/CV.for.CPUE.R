...
#' @return The function either plots the CPUE, together with a lowess line fitted to it, and returns the c.v. of the residuals to the fit. Or returns a dataframe of the lowess line fits and associated c.v.s for each point.
...
CV.for.CPUE <- function(year, cpue, f, plot.it = TRUE) {
  ...
  if (plot.it) {
    plot(year, cpue, pch = "x", ylim = c(0, 1.05 * max(c(cpue, lowess.fit$y))), yaxs = "i", xlab = "", ylab = "")
    lines(lowess.fit)
    return(CV)
  } else {
